...
let valid_float_lexeme s =
  ...
  let rec loop i =
    if i >= l
    then s ^ "."
    else (match s.[i] with | '0'..'9'|'-' -> loop (i + 1) | _ -> s)  in
  loop 0 
let float_repres f =
...
let print_out_value ppf tree =
  ...
  and print_simple_tree ppf =
    ...
    | Oval_string s ->
        (try fprintf ppf "\"%s\"" (Reason_syntax_util.escape_string s)
         with
    ...
    | Oval_tuple tree_list ->
        fprintf ppf "@[<1>(%a)@]" (print_tree_list print_tree_1 ",")
          tree_list
  ...
  and print_tree_list print_item sep ppf tree_list =
   ...
   in cautious print_tree_1 ppf tree 
  ...
  let suffix =
    ...
    | Optional lab ->
        (pp_print_string ppf "~";
         ...
         "=?")
     in
  print_out_type_2 ppf typ; pp_print_string ppf suffix

and print_out_type_1 ~uncurried  ppf =
  function
  | Otyp_arrow (lab,ty1,ty2) ->
      let rec collect_args args typ =
        match typ with
...
and print_simple_out_type ppf =
  ...
  | Otyp_class (ng,id,tyl) ->
      fprintf ppf "@[%s#%a%a@]" (if ng then "_" else "") print_ident id
        print_typargs tyl
  | Otyp_constr
      ((Oide_dot
          ((Oide_dot (Oide_ident "Js","Internal")|Oide_ident "Js_internal"),(
           "fn"|"meth" as name))
          as id),((Otyp_variant
                    (_,Ovar_fields ((variant,_,tys)::[]),_,_))::result::[] as
                    tyl))
      ->
      let make tys result =
        if tys = []
        then Otyp_arrow ("", (Otyp_constr ((Oide_ident "unit"), [])), result)
        else
          (match tys with
           | (Otyp_tuple tys as single)::[] ->
               ...
               then Otyp_arrow ("", single, result)
                 ...
                 List.fold_right
                   (fun x  -> fun acc  -> Otyp_arrow ("", x, acc)) tys result
      ...
      (match make tys result with
       ...
       | res ->
           (match name with
            ...
            | "meth" ->
                fprintf ppf "@[<0>(%a)@ [@bs.meth]@]"
                  (print_out_type_1 ~uncurried:false) res
            | _ -> assert false))
  | Otyp_constr
      ((Oide_dot
          ((Oide_dot (Oide_ident "Js","Internal")|Oide_ident "Js_internal"),"meth_callback")
          as id),((Otyp_variant
                    (_,Ovar_fields ((variant,_,tys)::[]),_,_))::result::[] as
                    tyl))
      ->
      let make tys result =
        ...
        | (Otyp_tuple tys as single)::[] ->
            ...
            then Otyp_arrow ("", single, result)
              ...
              List.fold_right (fun x  -> fun acc  -> Otyp_arrow ("", x, acc))
                tys result
        | single::[] -> Otyp_arrow ("", single, result)
      ...
      (match make tys result with
       | exception _ ->
           (pp_open_box ppf 0;
            ...
            pp_close_box ppf ())
       | res ->
           fprintf ppf "@[<0>(%a)@ [@bs.this]@]"
             (print_out_type_1 ~uncurried:false) res)
  | Otyp_constr
      (Oide_dot (Oide_ident "Js","t"),(Otyp_object (fields,rest))::[]) ->
      let dot =
        match rest with
        | Some non_gen -> (if non_gen then "_" else "") ^ ".."
        | None  -> "."  in
      fprintf ppf "@[<2>{%s %a}@]" dot
        (print_object_fields ~quote_fields:true) fields
  | Otyp_constr (id,tyl) ->
      (pp_open_box ppf 0;
       print_ident ppf id;
  ...
  | Otyp_object (fields,rest) ->
      let dot =
        ...
        | None  -> "."  in
      fprintf ppf "@[<2>{%s %a}@]" dot
        (print_object_fields ~quote_fields:false) fields
  ...
  | Otyp_variant (non_gen,row_fields,closed,tags) ->
      ...
      fprintf ppf "%s[%s@[<hv>@[<hv>%a@]%a ]@]" (if non_gen then "_" else "")
        (if closed
         ...
         else if tags = None then "> " else "? ") print_fields row_fields
        print_present tags
      ...
      (fprintf ppf "@[<1>(module %s" p;
       (let first = ref true  in
        List.iter2
          (fun s  ->
             fun t  ->
               let sep = if !first then (first := false; "with") else "and"
                  in
...
and print_object_fields ~quote_fields  ppf =
  ...
  | (field,typ)::rest ->
      let field = if quote_fields then "\"" ^ (field ^ "\"") else field  in
      fprintf ppf "%s: %a,@ %a" field print_out_type typ
        (print_object_fields ~quote_fields) rest

and print_row_field ppf (l,opt_amp,tyl) =
  let pr_of ppf = if opt_amp then fprintf ppf " &@ " else fprintf ppf ""  in
  let parens =
    match tyl with | (Otyp_tuple _)::[] -> false | _::[] -> true | _ -> false
     in
  fprintf ppf "@[<hv 2>`%s%t%s%a%s@]" l pr_of (if parens then "(" else "")
    (print_typlist print_out_type " &") tyl (if parens then ")" else "")
...
and print_out_wrap_type ppf =
  function
  | Otyp_constr (id,_::_) as ty -> print_out_type ppf ty
  | ty -> print_simple_out_type ppf ty
...
let print_out_class_params ppf =
  ...
  | tyl ->
      fprintf ppf "(@[<1>%a@])@ "
        (print_list type_parameter (fun ppf  -> fprintf ppf ",@ ")) tyl
...
let rec print_out_class_type ppf =
  ...
  | Octy_constr (id,tyl) ->
      let pr_tyl ppf =
        ...
        | [] -> ()
        | tyl ->
            fprintf ppf "@[<1> %a@]" (print_typlist print_out_wrap_type "")
              tyl
         in
      fprintf ppf "@[%a%a@]" print_ident id pr_tyl tyl
  | Octy_arrow (lab,argument_type,return_class_type) ->
      let rec print_class_type_arguments_that_might_be_arrow ppf =
        function
        | Otyp_arrow ("",typ1,typ2) ->
            fprintf ppf "@[%a,@ %a@]" print_out_type typ1
              print_class_type_arguments_that_might_be_arrow typ2
        | Otyp_arrow (actual_label,typ1,typ2) ->
            fprintf ppf "@[~%s: %a,@ %a@]" lab print_out_type typ1
              print_class_type_arguments_that_might_be_arrow typ2
        | argument_not_arrow ->
            fprintf ppf "%a" print_out_type argument_not_arrow
         in
      fprintf ppf "@[(%a) =>@ %a@]"
...
and print_out_class_sig_item ppf =
  ...
  | Ocsg_method (name,priv,virt,ty) ->
      fprintf ppf "@[<2>%s%s%s:@ %a@]" (if priv then "pri " else "pub ")
        (if virt then "virtual " else "") name print_out_type ty
  | Ocsg_value (name,mut,vr,ty) ->
      fprintf ppf "@[<2>val %s%s%s:@ %a@]" (if mut then "mutable " else "")
        (if vr then "virtual " else "") name print_out_type ty

let is_rec_next =
  function
  | (Osig_class (_,_,_,_,Orec_next ))::_|(Osig_class_type
    (_,_,_,_,Orec_next ))::_|(Osig_module (_,_,Orec_next ))::_|(Osig_type
    (_,Orec_next ))::_ -> true
  | _ -> false 
...
and print_out_signature ppf =
  ...
  | (Osig_typext (ext,Oext_first ))::items ->
      let rec gather_extensions acc items =
        ...
        | (Osig_typext (ext,Oext_next ))::items ->
            gather_extensions
              ...
              acc) items
        | _ -> ((List.rev acc), items)  in
      let (exts,items) =
        gather_extensions
          [((ext.oext_name), (ext.oext_args), (ext.oext_ret_type))] items
         in
      let te =
        {
      ...
      fprintf ppf "%a%s@ %a" print_out_type_extension te sep
        print_out_signature items
  | item::items ->
      let sep = if is_rec_next items then "" else ";"  in
      fprintf ppf "%a%s@ %a" print_out_sig_item item sep print_out_signature
        items
...
and print_out_sig_item ppf =
  ...
  | Osig_class (vir_flag,name,params,clt,rs) ->
      fprintf ppf "@[<2>%s%s@ %s %a@,:@ %a@]"
        ...
        (if vir_flag then " virtual" else "") name print_out_class_params
        params print_out_class_type clt
  ...
  | Osig_typext (ext,Oext_exception ) ->
      fprintf ppf "@[<2>exception %a@]" print_out_constr
        ((ext.oext_name), (ext.oext_args), (ext.oext_ret_type))
  | Osig_typext (ext,es) -> print_out_extension_constructor ppf ext
  | Osig_modtype (name,Omty_abstract ) ->
      fprintf ppf "@[<2>module type %s@]" name
  | Osig_modtype (name,mty) ->
      fprintf ppf "@[<2>module type %s =@ %a@]" name print_out_module_type
        mty
  ...
  | Osig_value { oval_name; oval_type; oval_prims; oval_attributes } ->
      ...
      fprintf ppf "@[<2>%a%a%s %a:@ %a%a@]"
        (fun ppf  -> List.iter (fun _  -> fprintf ppf "[@@bs...]@ "))
        hackyBucklescriptExternalAnnotation printAttributes oval_attributes
        keyword value_ident oval_name (!out_type) oval_type
...
and print_out_type_decl kwd ppf td =
    ...
    List.iter
      (fun (ty1,ty2)  ->
         fprintf ppf "@ @[<2>constraint %a =@ %a@]" print_out_type ty1
           print_out_type ty2) td.otype_cstrs
     in
  ...
  let print_name_params ppf =
    fprintf ppf "%s %t%a" kwd type_defined print_manifest td.otype_type  in
  let ty =
    match td.otype_type with
    | Otyp_manifest (_,ty) -> ty
    | _ -> td.otype_type  in
  let print_private ppf =
    function
  ...
  let print_out_tkind ppf =
    ...
    | Otyp_abstract  -> ()
    | Otyp_record lbls ->
        fprintf ppf " =%a {%a@;<1 -2>}" print_private td.otype_private
    ...
    | ty ->
        fprintf ppf " =%a@;<1 2>%a" print_private td.otype_private
          print_out_type ty
     in
  fprintf ppf "@[<2>@[<hv 2>%t%a@]%t@]" print_name_params print_out_tkind ty
    print_constraints
...
and print_out_extension_constructor ppf ext =
  let print_extended_type ppf =
    let print_type_parameter ppf ty =
      fprintf ppf "%s" (if ty = "_" then ty else "'" ^ ty)  in
    match ext.oext_type_params with
    | [] -> fprintf ppf "%s" ext.oext_type_name
    | ty_param::[] ->
        fprintf ppf "@[%a@ %s@]" print_type_parameter ty_param
          ext.oext_type_name
    | _ ->
        fprintf ppf "@[(@[%a)@]@ %s@]"
