...
let pr_white_str spaces ppf s =
  ...
  while (!right) <> len do
    if (s.[!right]) = '\n'
    then (flush (); Format.pp_force_newline ppf ())
    else
      if spaces && ((s.[!right]) = ' ')
      then (flush (); Format.pp_print_space ppf ())
      else incr right
    done;
  if (!left) <> len then flush () 
let pr_text = pr_white_str true 
let pr_lines = pr_white_str false 
let pr_to_temp_file pr v =
  try
...
let suggest s candidates =
  let add (min,acc) name =
    ...
    if d = min
    then (min, (name :: acc))
    else if d < min then (d, [name]) else (min, acc)  in
  let (dist,suggs) = List.fold_left add (max_int, []) candidates  in
  if dist < 3 then suggs else [] 
module Trie :
  sig
    type 'a t
    val empty : 'a t
      ...
      let rec aux t k len i d pre_d =
        ...
        then { v = (Key d); succs = (t.succs) }
          ...
          (let v =
           ...
           let succs =
             let t' =
               try Cmap.find (k.[i]) t.succs with | Not_found  -> empty  in
             Cmap.add (k.[i]) (aux t' k len (i + 1) d pre_d) t.succs  in
           { v; succs })
         in
      aux t k (String.length k) 0 d (Pre d) 
    let find_node t k =
      let rec aux t k len i =
        if i = len then t else aux (Cmap.find (k.[i]) t.succs) k len (i + 1)
         in
      aux t k (String.length k) 0 
    let find t k =
      try
        match (find_node t k).v with
        | Key v|Pre v -> `Ok v
        | Amb  -> `Ambiguous
        | Nil  -> `Not_found
    ...
    let ambiguities t p =
      try
        ...
        | Amb  ->
            ...
            let to_list m =
              Cmap.fold (fun k  -> fun t  -> fun acc  -> (k, t) :: acc) m []
               in
            let rec aux acc p =
              function
              | ((c,t)::succs)::rest ->
      ...
      with | Not_found  -> [] 
    let of_list l =
      List.fold_left (fun t  -> fun (s,v)  -> add t s v) empty l 
  end 
type env_info = {
  env_var: string ;
  env_doc: string ;
...
let arg_id =
  let c = ref 0  in
  fun ()  -> let id = !c  in incr c; if id > (!c) then assert false else id 
...
type man_block =
  [ `S of string  | `P of string  | `Pre of string 
  | `I of (string * string)  | `Noblank ]
type term_info =
  {
  name: string ;
...
let eval_kind ei =
  if ei.choices = []
  then `Simple
  else if (fst ei.term) == (fst ei.main) then `M_main else `M_choice 
module Manpage =
  struct
    ...
    let escape subst esc buf s =
      let subst s =
        ...
        if not ((len > 1) && ((s.[1]) = ','))
        then subst s
        else if len = 2 then "" else esc (s.[0]) (String.sub s 2 (len - 2))
         in
      try
        Buffer.clear buf;
        Buffer.add_substitute buf subst s;
      ...
      let rec aux =
        ...
        | t::ts ->
            ((match t with
              ...
              | `Pre s ->
                  pr ppf "%a@[%a@]@," pr_indent p_indent pr_lines (escape s)
              | `I (label,s) ->
                  ...
                  (pr ppf "@[%a@[%a@]" pr_indent p_indent pr_tokens label;
                   ...
                   then ()
                     ...
                     if ll < l_indent
                     then
                       pr ppf "%a@[%a@]@]@," pr_indent (l_indent - ll)
                         pr_tokens s
                     else
                       pr ppf "@\n%a@[%a@]@]@," pr_indent
                         (p_indent + l_indent) pr_tokens s));
             (match ts with
              | `Noblank::ts -> aux ts
    ...
    let find_cmd cmds =
      let (test,null) =
        ...
        | "Win32" -> ("where", " NUL")
        | _ -> ("type", "/dev/null")  in
      let cmd c = (Sys.command (str "%s %s 1>%s 2>%s" test c null null)) = 0
         in
      try Some (List.find cmd cmds) with | Not_found  -> None 
    let pr_to_pager print ppf v =
      let pager =
        let cmds = ["less"; "more"]  in
        let cmds = try (Sys.getenv "PAGER") :: cmds with | Not_found  -> cmds
           in
        let cmds =
          try (Sys.getenv "MANPAGER") :: cmds with | Not_found  -> cmds  in
        find_cmd cmds  in
      match pager with
      | None  -> print `Plain ppf v
      | Some pager ->
          let cmd =
            match find_cmd ["groff"; "nroff"] with
            | None  ->
                (match pr_to_temp_file (print `Plain) v with
                 | None  -> None
                 | Some f -> Some (str "%s < %s" pager f))
            | Some c ->
                (match pr_to_temp_file (print `Groff) v with
                 | None  -> None
                 | Some f ->
                     let xroff =
                       if c = "groff" then c ^ " -Tascii -P-c" else c  in
    ...
    let rec print ?(subst= fun x  -> x)  fmt ppf page =
      match fmt with
...
module Help =
  struct
    ...
    let title ei =
      ...
      let left_footer =
        prog ^
          (match (fst ei.main).version with
           | None  -> ""
           | Some v -> str " %s" v)
         in
      ...
      | `Simple|`M_choice ->
          let rev_cmp (p,_) (p',_) =
            ...
            | (Left (false ,k),Nth (false ,k'))
              ...
              |(Nth (false ,k),Right (false ,k')) ->
                if k <= k' then (-1) else 1
            | (Nth (false ,k),Left (false ,k'))
              |(Right (false ,k),Nth (false ,k')) ->
                if k >= k' then 1 else (-1)
            | (Left (true ,k),Nth (true ,k'))|(Nth (true ,k),Nth (true ,k'))
              |(Nth (true ,k),Right (true ,k')) ->
                if k >= k' then (-1) else 1
            | (Nth (true ,k),Left (true ,k'))
              |(Right (true ,k),Nth (true ,k')) ->
                if k <= k' then 1 else (-1)
            | (p,p') -> compare p p'  in
          let rec format_pos acc =
            function
            | a::al ->
                if is_opt a
                then format_pos acc al
                else
                  (let v =
                     if a.docv = "" then "$(i,ARG)" else str "$(i,%s)" a.docv
                      in
                   let v =
                     if a.absent = Error then str "%s" v else str "[%s]" v
                      in
                   let v =
                     v ^ (match a.p_kind with | Nth _ -> "" | _ -> "...")  in
    ...
    let get_synopsis_section ei =
      let rec extract_synopsis syn =
        function
        | (`S _)::_ as man -> ((List.rev syn), man)
        | block::rest -> extract_synopsis (block :: syn) rest
        | [] -> ((List.rev syn), [])  in
      match (fst ei.term).man with
      | (`S "SYNOPSIS" as s)::rest -> extract_synopsis [s] rest
      | man -> ([`S "SYNOPSIS"; `P (synopsis ei)], man) 
    let or_env a =
      match a.env_info with
      | None  -> ""
        ...
        (let fmt_name var =
           ...
           | Opt  ->
               (fun n  ->
                  ...
                  then str "$(b,%s)=$(i,%s)" n var
                  else str "$(b,%s) $(i,%s)" n var)
           | Opt_vopt _ ->
               (fun n  ->
                  if (String.length n) > 2
                  then str "$(b,%s)[=$(i,%s)]" n var
         ...
         let s = String.concat ", " (List.rev_map (fmt_name var) names)  in s)
    ...
    let arg_info_substs ~buf  a doc =
      let subst =
        ...
        | "docv" -> str "$(i,%s)" a.docv
        | "opt" when is_opt a ->
            let k = String.lowercase (List.hd (List.sort compare a.o_names))
               in
            str "$(b,%s)" k
        ...
        then c
          ...
          (match ((is_opt a), (is_opt a')) with
           | (true ,true ) ->
               let key names =
                 let k =
                   String.lowercase (List.hd (List.sort rev_compare names))
                    in
                 if (k.[1]) = '-'
      ...
      let format a =
        let absent =
          ...
          | Val v ->
           ...
           in
        let optvopt =
          match a.o_kind with | Opt_vopt v -> str "default=%s" v | _ -> ""
           in
        let argvdoc =
          match (optvopt, absent) with
          | ("","") -> ""
          | (s,"")|("",s) -> str " (%s)" s
          | (s,s') -> str " (%s) (%s)" s s'  in
        ((a.docs),
          (`I
             (((make_arg_label a) ^ argvdoc), (arg_info_substs ~buf a a.doc))))
         in
      let is_arg_item a = not ((is_pos a) && ((a.docv = "") || (a.doc = "")))
         in
    ...
    let make_env_items_rev ei =
      ...
      let cmp a a' =
        let e' =
          match a'.env_info with | None  -> assert false | Some a' -> a'  in
        let e = match a.env_info with | None  -> assert false | Some a -> a
           in
        let c = compare e.env_docs e'.env_docs  in
        if c <> 0 then c else compare e.env_var e'.env_var  in
      let format a =
        let e = match a.env_info with | None  -> assert false | Some a -> a
           in
        ((e.env_docs),
    ...
    let text ei =
      let rec merge_items acc to_insert mark il =
        ...
        | [] ->
         ...
         in
      let rec merge_orphans acc orphans =
        function
        | `Orphan_mark::ts ->
            let rec merge acc s =
              function
              ...
              | (s',i)::ss ->
                  ...
                  if s = s'
                  then merge (i :: acc) s ss
                  else merge (i :: (`S s) :: acc) s' ss
               in
            let acc =
              match orphans with
              | [] -> acc
              | (s,_)::_ -> merge acc s orphans  in
      ...
      let items_rev = List.rev_append cmds (List.rev_append args envs_rev)
         in
      let cmp (s,_) (s',_) =
        match (s, s) with
        | ("ENVIRONMENT VARIABLES",_) -> 1
        | (s,"ENVIRONMENT VARIABLES") -> (-1)
        | (s,s') -> compare s s'  in
      let items = List.rev (List.stable_sort cmp items_rev)  in
      let (synopsis,man) = get_synopsis_section ei  in
      let (rev_text,orphans) = merge_items [`Orphan_mark] [] false items man
         in
    ...
    let print fmt ppf ei =
      Manpage.print ~subst:(ei_subst ei) fmt ppf (man ei) 
    let pr_synopsis ppf ei =
      pr ppf "@[%s@]"
        (Manpage.escape (ei_subst ei) Manpage.plain_esc (Buffer.create 100)
           (synopsis ei))
      
    let pr_version ppf ei =
      match (fst ei.main).version with
      | None  -> assert false
...
module Err =
  struct
    ...
    let element kind s exp =
      str "invalid element in %s (`%s'): %s" kind s exp 
    ...
    let unknown kind ?(hints= [])  v =
      ...
      let hints =
        match hints with | [] -> "." | hs -> did_you_mean (alts_str hs)  in
    ...
    let flag_value f v =
      str "option %s is a flag, it cannot take the argument %s" (quote f)
        (quote v)
      
    let opt_value_missing f = str "option %s needs an argument" (quote f) 
    let opt_parse_value f e = str "option %s: %s" (quote f) e 
    let env_parse_value var e =
      str "environment variable %s: %s" (quote var) e 
    let opt_repeated f f' =
      if f = f'
      then str "option %s cannot be repeated" (quote f)
      else
    ...
    let arg_missing a =
      ...
      then
        let rec long_name =
          function
          | n::l ->
              if ((String.length n) > 2) || (l = []) then n else long_name l
          | [] -> assert false  in
        str "required option %s is missing" (long_name a.o_names)
      else
        if a.docv = ""
        then str "a required argument is missing"
        else str "required argument %s is missing" a.docv
      
    let print ppf ei e = pr ppf "%s: @[%a@]@." (fst ei.main).name pr_text e 
    let pr_backtrace err ei e bt =
      let bt =
        let len = String.length bt  in
        if len > 0 then String.sub bt 0 (len - 1) else bt  in
      pr err "%s: @[internal error, uncaught exception:@\n%a@]@."
    ...
    let pr_try_help ppf ei =
      ...
      if exec = main
      then pr ppf "@[<2>Try `%s --help' for more information.@]" exec
      else
        pr ppf "@[<2>Try `%s --help' or `%s --help' for more information.@]"
          exec main
      
    let pr_usage ppf ei e =
      pr ppf "@[<v>%s: @[%a@]@,@[Usage: @[%a@]@]@,%a@]@." (fst ei.main).name
        pr_text e Help.pr_synopsis ei pr_try_help ei
...
module Cmdline :
  sig
    ...
    val choose_term :
      term_info ->
        (term_info * 'a) list -> string list -> (term_info * string list)
  ...
  struct
    exception Error of string 
    let opt_arg cl a =
      match try Amap.find a cl with | Not_found  -> assert false with
      | O l -> l
      | _ -> assert false 
    let pos_arg cl a =
      match try Amap.find a cl with | Not_found  -> assert false with
      | P l -> l
      | _ -> assert false 
    let choose_term ti choices =
      function
      | [] -> (ti, [])
      | maybe::args' as args ->
          if ((String.length maybe) > 1) && ((maybe.[0]) = '-')
          then (ti, args)
          else
            (let index =
               let add acc (choice,_) = Trie.add acc choice.name choice  in
               List.fold_left add Trie.empty choices  in
             ...
             | `Ambiguous ->
                 let ambs = List.sort compare (Trie.ambiguities index maybe)
                    in
                 raise (Error (Err.ambiguous "command" maybe ambs)))
      
    let arg_info_indexes al =
      let rec aux opti posi cl =
        function
        | a::l ->
            if is_pos a
            then aux opti (a :: posi) (Amap.add a (P []) cl) l
            else
              (let add t name = Trie.add t name a  in
               aux (List.fold_left add opti a.o_names) posi
                 (Amap.add a (O []) cl) l)
        | [] -> (opti, posi, cl)  in
      ...
      let rec aux k opti cl pargs =
        ...
        | s::args ->
            ...
            let is_short_opt s = ((String.length s) = 2) && ((s.[0]) = '-')
               in
            if not (is_opt s)
            then aux (k + 1) opti cl (s :: pargs) args
            else
              (let (name,value) = parse_opt_arg s  in
               match Trie.find opti name with
               | `Ok a ->
                   let (value,args) =
                     match (value, (a.o_kind)) with
                     | (Some v,Flag ) when is_short_opt name ->
                         (None, (("-" ^ v) :: args))
                     | (Some v,_) -> (value, args)
               ...
               | `Not_found when peek_opts -> aux (k + 1) opti cl pargs args
               | `Not_found ->
                   let hints =
                     ...
                     then []
                       ...
                       (let (short_opt,long_opt) =
                        ...
                        match ((List.mem short_opt all),
                                (suggest long_opt all))
                        with
                        | (false ,[]) -> []
                        | (false ,l) -> l
                        | (true ,[]) -> [short_opt]
                        | (true ,l) ->
                            if List.mem short_opt l
                            then l
                            else short_opt :: l)
                      in
                   raise (Error (Err.unknown "option" ~hints name))
               | `Ambiguous ->
                   let ambs = List.sort compare (Trie.ambiguities opti name)
                      in
    ...
    let process_pos_args posi cl pargs =
      ...
      then cl
        ...
        (let rec take n acc l =
           if n = 0
           then List.rev acc
           else take (n - 1) ((List.hd l) :: acc) (List.tl l)  in
         let rec aux pargs last cl max_spec =
           function
           | a::al ->
               let (arg,max_spec) =
                 match a.p_kind with
                 ...
                 | Nth (rev,k) ->
                     ...
                     if (k < 0) || (k > last)
                     then ((P []), max_spec)
                     else ((P [List.nth pargs k]), max_spec)
                 | Left (rev,k) ->
                     let k = if rev then last - k else k  in
                     let max_spec = max k max_spec  in
                     if (k <= 0) || (k > last)
                     then ((P []), max_spec)
                     else ((P (take k [] pargs)), max_spec)
                 | Right (rev,k) ->
                     let k = if rev then last - k else k  in
                     if (k < 0) || (k >= last)
                     then ((P []), last)
                     else
                       ((P (List.rev (take (last - k) [] (List.rev pargs)))),
                         last)
                  in
         ...
         then cl
           ...
           (let excess =
              List.rev (take (last - max_spec) [] (List.rev pargs))  in
    ...
    let create ?(peek_opts= false)  al args =
      ...
      if peek_opts then cl else process_pos_args posi cl pargs 
  end 
module Arg =
  struct
    ...
    type info = arg_info
    let env_var ?(docs= "ENVIRONMENT VARIABLES")  ?(doc=
      "See option $(opt).")  env_var =
      { env_var; env_doc = doc; env_docs = docs } 
    let (&) f x = f x 
    let parse_error e = raise (Cmdline.Error e) 
    let some ?(none= "")  (parse,print) =
      ((fun s  ->
          match parse s with | `Ok v -> `Ok (Some v) | `Error _ as e -> e),
        (fun ppf  ->
           fun v  ->
             match v with | None  -> pr_str ppf none | Some v -> print ppf v))
    ...
    let env_bool_parse s =
      match String.lowercase s with
      | ""|"false"|"no"|"n"|"0" -> `Ok false
      | "true"|"yes"|"y"|"1" -> `Ok true
      | s ->
          `Error
            (Err.invalid_val s (alts_str ["true"; "yes"; "false"; "no"]))
      
    let parse_to_list parser s =
      match parser s with | `Ok v -> `Ok [v] | `Error _ as e -> e 
    let try_env ei a parse ~absent  =
      match a.env_info with
      | None  -> absent
      | Some env ->
          (match ei.env env.env_var with
           ...
           | Some v ->
               (match parse v with
                | `Ok v -> v
                | `Error e -> parse_error (Err.env_parse_value env.env_var e)))
    ...
    let flag a =
      ...
      then invalid_arg err_not_opt
        ...
        (let convert ei cl =
           ...
           | (_,f,_)::(_,g,_)::_ -> parse_error (Err.opt_repeated f g)  in
         ([a], convert))
         ...
         let convert ei cl =
           ...
           | l ->
               let truth (_,f,v) =
                 match v with
                 | None  -> true
                 | Some v -> parse_error (Err.flag_value f v)  in
    ...
    let vflag v l =
      let convert _ cl =
        let rec aux fv =
          ...
          | [] -> (match fv with | None  -> v | Some (_,v) -> v)  in
        aux None l  in
      ...
      let convert _ cl =
        let rec aux acc =
          ...
          | [] ->
              if acc = []
              then v
              else List.rev_map snd (List.sort rev_compare acc)
           in
        aux [] l  in
      let flag (_,a) =
        if is_pos a then invalid_arg err_not_opt else { a with o_all = true }
         in
      ((List.rev_map flag l), convert) 
    let parse_opt_value parse f v =
      match parse v with
      | `Ok v -> v
      | `Error e -> parse_error (Err.opt_parse_value f e) 
        ...
        (let a =
           {
             ...
             o_kind =
               (match vopt with
                | None  -> Opt
                | Some dv -> Opt_vopt (str_of_pp print dv))
           }  in
         let convert ei cl =
           match Cmdline.opt_arg cl a with
           ...
           | (_,f,None )::[] ->
               (match vopt with
                ...
                | Some optv -> optv)
           | (_,f,_)::(_,g,_)::_ -> parse_error (Err.opt_repeated g f)  in
         ([a], convert))
           ...
           | l ->
               let parse (k,f,v) =
                 ...
                 | None  ->
                  ...
                  in
               List.rev_map snd
                 (List.sort rev_compare (List.rev_map parse l))
            in
         ([a], convert))
      
    let parse_pos_value parse a v =
      match parse v with
      | `Ok v -> v
      | `Error e -> parse_error (Err.pos_parse_value a e) 
    let pos ?(rev= false)  k (parse,print) v a =
      if is_opt a
      then invalid_arg err_not_pos
      else
        (let a =
           {
             a with
         ...
         let convert ei cl =
           match Cmdline.pos_arg cl a with
           | [] -> try_env ei a parse ~absent:v
      ...
      then invalid_arg err_not_pos
        ...
        (let a = { a with p_kind = kind }  in
         let convert ei cl =
           match Cmdline.pos_arg cl a with
           | [] -> try_env ei a (parse_to_list parse) ~absent:v
           | l -> List.rev (List.rev_map (parse_pos_value parse a) l)  in
         ([a], convert))
    ...
    let absent_error al =
      List.rev_map (fun a  -> { a with absent = Error }) al 
    let value a = a 
    let required (al,convert) =
      let al = absent_error al  in
      let convert ei cl =
        match convert ei cl with
        | Some v -> v
        | None  -> parse_error (Err.arg_missing (List.hd al))  in
      (al, convert) 
    ...
    let char =
      ((fun s  ->
          ...
          then `Ok (s.[0])
          else `Error (Err.invalid_val s "expected a character")), pr_char)
    ...
    let enum sl =
      if sl = []
      then invalid_arg err_empty_list
      else
        (let t = Trie.of_list sl  in
         let parse s =
           match Trie.find t s with
           | `Ok _ as r -> r
           | `Ambiguous ->
         ...
         let print ppf v =
           ...
           with | Not_found  -> invalid_arg err_incomplete_enum  in
         (parse, print))
    ...
    let file =
      ((fun s  ->
          ...
          then `Ok s
          else `Error (Err.no "file or directory" s)), pr_str)
      
    let dir =
      ((fun s  ->
          if Sys.file_exists s
          then (if Sys.is_directory s then `Ok s else `Error (Err.not_dir s))
          else `Error (Err.no "directory" s)), pr_str)
      ...
      ((fun s  ->
          ...
          then
            (if not (Sys.is_directory s)
             then `Ok s
             else `Error (Err.is_dir s))
          else `Error (Err.no "file" s)), pr_str)
      
    let split_and_parse sep parse s =
      let parse sub =
        match parse sub with | `Error e -> failwith e | `Ok v -> v  in
    ...
    let list ?(sep= ',')  (parse,pr_e) =
      let parse s =
        try `Ok (split_and_parse sep parse s)
        with | Failure e -> `Error (Err.element "list" s e)  in
      let rec print ppf =
        function
        | v::l ->
            (pr_e ppf v; if l <> [] then (pr_char ppf sep; print ppf l))
        | [] -> ()  in
      (parse, print) 
    let array ?(sep= ',')  (parse,pr_e) =
      let parse s =
        try `Ok (Array.of_list (split_and_parse sep parse s))
        with | Failure e -> `Error (Err.element "array" s e)  in
      let print ppf v =
        let max = (Array.length v) - 1  in
        for i = 0 to max do
          pr_e ppf (v.(i)); if i <> max then pr_char ppf sep
        done  in
      (parse, print) 
    ...
    let t3 ?(sep= ',')  (pa0,pr0) (pa1,pr1) (pa2,pr2) =
      ...
      let print ppf (v0,v1,v2) =
        pr ppf "%a%c%a%c%a" pr0 v0 sep pr1 v1 sep pr2 v2  in
      (parse, print) 
                  ...
                  | Some (v2,v3) ->
                      (match ((pa0 v0), (pa1 v1), (pa2 v2), (pa3 v3)) with
                       ...
                       | (`Error e,_,_,_)|(_,`Error e,_,_)|(_,_,`Error e,_)
                         |(_,_,_,`Error e) ->
                           `Error (Err.element "quadruple" s e))))
         in
      let print ppf (v0,v1,v2,v3) =
        pr ppf "%a%c%a%c%a%c%a" pr0 v0 sep pr1 v1 sep pr2 v2 sep pr3 v3  in
      (parse, print) 
    let doc_quote = quote 
...
module Term =
  struct
    ...
    type 'a result =
      [ `Ok of 'a  | `Error of [ `Parse  | `Term  | `Exn ]  | `Version 
      | `Help ]
    exception Term of
    ...
    let info ?(sdocs= "OPTIONS")  ?(man= [])  ?(docs= "COMMANDS")  ?(doc= "")
       ?version  name =
      { name; version; tdoc = doc; tdocs = docs; sdocs; man } 
    let name ti = ti.name 
    let const v = ([], (fun _  -> fun _  -> v)) 
    let pure = const 
    let app (al,f) (al',v) =
      ((List.rev_append al al'), (fun ei  -> fun cl  -> (f ei cl) (v ei cl))) 
    let ($) = app 
    type 'a ret =
      [ `Help of ([ `Pager  | `Plain  | `Groff ] * string option) 
      | `Error of (bool * string)  | `Ok of 'a ]
    let ret (al,v) =
      (al,
        (fun ei  ->
           fun cl  ->
    ...
    let main_name = ([], (fun ei  -> fun _  -> (fst ei.main).name)) 
    let choice_names =
      ([],
        (fun ei  ->
           fun _  -> List.rev_map (fun e  -> (fst e).name) ei.choices))
      
    let man_format =
      let fmts = [("pager", `Pager); ("groff", `Groff); ("plain", `Plain)]
         in
      let doc = "Show output in format $(docv) (pager, plain or groff)."  in
      let open Arg in
        value &
          ((opt (enum fmts) `Pager) & (info ["man-format"] ~docv:"FMT" ~doc))
      
    let remove_exec argv =
      try List.tl (Array.to_list argv)
      with | Failure _ -> invalid_arg err_argv 
    let add_std_opts ei =
      let docs = (fst ei.term).sdocs  in
      let (args,v_lookup) =
        if (fst ei.main).version = None
        then ([], None)
        else
          (let (a,lookup) =
             Arg.flag
               (Arg.info ["version"] ~docs ~doc:"Show version information.")
              in
           (a, (Some lookup)))
         in
      let (args,h_lookup) =
        let (a,lookup) =
          let fmt =
            Arg.enum
              [("pager", `Pager); ("groff", `Groff); ("plain", `Plain)]
             in
          let doc =
            "Show this help in format $(docv) (pager, plain or groff)."  in
          let a = Arg.info ["help"] ~docv:"FMT" ~docs ~doc  in
          Arg.opt ~vopt:(Some `Pager) (Arg.some fmt) None a  in
        ((List.rev_append a args), lookup)  in
      (h_lookup, v_lookup,
        {
          ei with
          term = ((fst ei.term), (List.rev_append args (snd ei.term)))
        })
    ...
    let eval_term help err ei f args =
      ...
      try
        ...
        | (None ,Some v_arg) when v_arg ei cl ->
            (Help.pr_version help ei; `Version)
        | _ -> `Ok (f ei cl)
      with | Cmdline.Error e -> (Err.pr_usage err ei e; `Error `Parse)
      | Term (`Error (usage,e)) ->
          ...
          let ei =
            ...
            | Some cmd ->
                let cmd =
                  ...
                  with | Not_found  -> invalid_arg (err_help cmd)  in
                { ei with term = cmd }
            | None  -> { ei with term = (ei.main) }  in
          let (_,_,ei) = add_std_opts ei  in (Help.print fmt help ei; `Help)
    ...
    let eval ?(help= Format.std_formatter)  ?(err= Format.err_formatter) 
      ...
      let term = (ti, al)  in
      let ei = { term; main = term; choices = []; env }  in
      try eval_term help err ei f (remove_exec argv)
      with
      | e when catch ->
          (Err.pr_backtrace err ei e (Printexc.get_backtrace ()); `Error `Exn)
      
    let eval_choice ?(help= Format.std_formatter)  ?(err=
      Format.err_formatter)  ?(catch= true)  ?(env= env_default)  ?(argv=
      Sys.argv)  (((al,f) as t),ti) choices =
      let ei_choices = List.rev_map (fun ((al,_),ti)  -> (ti, al)) choices
         in
      let main = (ti, al)  in
      let ei = { term = main; main; choices = ei_choices; env }  in
      try
        let (chosen,args) =
          Cmdline.choose_term ti ei_choices (remove_exec argv)  in
    ...
    let eval_peek_opts ?(version_opt= false)  ?(env= env_default)  ?(argv=
      Sys.argv)  (al,f) =
      ...
      try
        ...
        | _ -> let v = f ei cl  in ((Some v), (`Ok v))
      with | Cmdline.Error _ -> (None, (`Error `Parse))
      | Term _ -> (None, (`Error `Term)) | e -> (None, (`Error `Exn)) 
  end