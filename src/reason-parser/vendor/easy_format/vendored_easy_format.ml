open Format
type wrap =
  [ `Wrap_atoms  | `Always_wrap  | `Never_wrap  | `Force_breaks 
  | `Force_breaks_rec  | `No_breaks ]
...
let propagate_from_leaf_to_root ~init_acc  ~merge_acc  ~map_node  x =
  let rec aux x =
    match x with
    | Atom _ -> let acc = init_acc x  in map_node x acc
    | List (param,children) ->
    ...
    | Custom _ -> let acc = init_acc x  in map_node x acc  in
  aux x 
let propagate_forced_breaks x =
  let init_acc =
    function
    | List ((_,_,_,{ wrap_body = `Force_breaks_rec }),_)|Label
      ((_,{ label_break = `Always_rec }),_) -> true
    | Atom _|Label _|Custom _|List _ -> false  in
  ...
  let map_node x force_breaks =
    ...
    | List
        ((op,sep,cl,({ wrap_body = (`Wrap_atoms|`Never_wrap|`Always_wrap) }
                       as p)),children)
        ->
        if force_breaks
        then
    ...
    | List ((_,_,_,{ wrap_body = `No_breaks }),_)|Label
      ((_,{ label_break = (`Always|`Always_rec|`Never) }),_)|Atom _|Custom _
        -> (x, force_breaks)
     in
  let (new_x,forced_breaks) =
    propagate_from_leaf_to_root ~init_acc ~merge_acc ~map_node x  in
  new_x 
module Pretty =
  struct
    ...
    let set_escape fmt escape =
      ...
      let mot tag = is_tag := true; tagf0.mark_open_tag tag  in
      let mct tag = is_tag := true; tagf0.mark_close_tag tag  in
      let print s p n =
        if !is_tag
        then (print0 s p n; is_tag := false)
        else escape print0 s p n  in
      let tagf = { tagf0 with mark_open_tag = mot; mark_close_tag = mct }  in
    ...
    let set_escape_string fmt esc =
      let escape print s p n =
        ...
        let s1 = esc s0  in print s1 0 (String.length s1)  in
         ...
         (let tbl1 = Hashtbl.create (2 * (List.length l))  in
          ...
          List.iter
            (fun (style_name,style)  ->
               ...
               Hashtbl.add tbl2 style_name style.tag_close) l;
          (let mark_open_tag style_name =
             try Hashtbl.find tbl1 style_name with | Not_found  -> ""  in
           let mark_close_tag style_name =
             try Hashtbl.find tbl2 style_name with | Not_found  -> ""  in
           let tagf =
             {
    ...
    let pp_open_xbox fmt p indent =
      ...
      | `Always_wrap|`Never_wrap|`Wrap_atoms -> pp_open_hvbox fmt indent
      | `Force_breaks|`Force_breaks_rec -> pp_open_vbox fmt indent
      | `No_breaks -> pp_open_hbox fmt () 
    let extra_box p l =
      let wrap =
        match p.wrap_body with
        | `Always_wrap -> true
        | `Never_wrap|`Force_breaks|`Force_breaks_rec|`No_breaks -> false
        | `Wrap_atoms ->
      ...
      else (((fun fmt  -> ())), ((fun fmt  -> ()))) 
    let pp_open_nonaligned_box fmt p indent l =
      match p.wrap_body with
      | `Always_wrap -> pp_open_hovbox fmt indent
      | `Never_wrap -> pp_open_hvbox fmt indent
      | `Wrap_atoms ->
          if List.for_all (function | Atom _ -> true | _ -> false) l
          then pp_open_hovbox fmt indent
          else pp_open_hvbox fmt indent
      | `Force_breaks|`Force_breaks_rec -> pp_open_vbox fmt indent
      | `No_breaks -> pp_open_hbox fmt () 
    let open_tag fmt = function | None  -> () | Some s -> pp_open_tag fmt s 
    let close_tag fmt =
      function | None  -> () | Some _ -> pp_close_tag fmt () 
    let tag_string fmt o s =
      match o with
      | None  -> pp_print_string fmt s
      | Some tag ->
          (pp_open_tag fmt tag; pp_print_string fmt s; pp_close_tag fmt ())
    ...
    let rec fprint_t fmt =
      ...
      | List (((_,_,_,p) as param),l) ->
          (open_tag fmt p.list_style;
           if p.align_closing
           then fprint_list fmt None param l
           else fprint_list2 fmt param l;
    ...
    and fprint_list_body_stick_left fmt p sep hd tl =
      ...
      List.iter
        (fun x  ->
           if p.space_before_separator then pp_print_string fmt " ";
           tag_string fmt p.separator_style sep;
           if p.space_after_separator
           then pp_print_space fmt ()
    ...
    and fprint_list_body_stick_right fmt p sep hd tl =
      ...
      List.iter
        (fun x  ->
           ...
           tag_string fmt p.separator_style sep;
           if p.space_after_separator then pp_print_string fmt " ";
           fprint_t fmt x) tl;
      close_tag fmt p.body_style
    ...
    and fprint_opt_label fmt =
      ...
      | Some (lab,lp) ->
          (open_tag fmt lp.label_style;
           ...
           if lp.space_after_label then pp_print_string fmt " ")
    
    and fprint_list fmt label ((op,sep,cl,p) as param) =
      function
      | [] ->
      ...
      | hd::tl as l ->
          ...
          then fprint_list_stick_left fmt label param hd tl l
          else fprint_list_stick_right fmt label param hd tl l
    ...
    and fprint_list_stick_left fmt label (op,sep,cl,p) hd tl l =
      ...
      (let (open_extra,close_extra) = extra_box p l  in
       ...
       then pp_print_break fmt 1 (- indent)
       else pp_print_break fmt 0 (- indent);
       tag_string fmt p.closing_style cl;
       pp_close_box fmt ())
    ...
    and fprint_list_stick_right fmt label (op,sep,cl,p) hd tl l =
      ...
      let sep_indent =
        (String.length sep) + (if p.space_after_separator then 1 else 0)  in
      ...
      (let (open_extra,close_extra) = extra_box p l  in
       ...
       List.iter
         (fun x  ->
            if p.space_before_separator
            then pp_print_break fmt 1 (- sep_indent)
            else pp_print_break fmt 0 (- sep_indent);
            tag_string fmt p.separator_style sep;
            if p.space_after_separator then pp_print_string fmt " ";
            fprint_t fmt x) tl;
       close_extra fmt;
    ...
    and fprint_list2 fmt (op,sep,cl,p) =
      ...
      | hd::tl as l ->
          (tag_string fmt p.opening_style op;
           if p.space_after_opening then pp_print_string fmt " ";
           pp_open_nonaligned_box fmt p 0 l;
           if p.separators_stick_left
           then fprint_list_body_stick_left fmt p sep hd tl
           else fprint_list_body_stick_right fmt p sep hd tl;
           pp_close_box fmt ();
           if p.space_before_closing then pp_print_string fmt " ";
           tag_string fmt p.closing_style cl)
      ...
      | _ ->
          ...
          (pp_open_hvbox fmt 0;
           ...
           (match lp.label_break with
            ...
            | `Never ->
                if lp.space_after_label then pp_print_char fmt ' ' else ());
           fprint_t fmt x;
           pp_close_box fmt ())
    
    let to_formatter fmt x =
      let x = rewrite x  in fprint_t fmt x; pp_print_flush fmt () 
    let to_buffer ?(escape= `None)  ?(styles= [])  buf x =
      let fmt = Format.formatter_of_buffer buf  in
      define_styles fmt escape styles; to_formatter fmt x 
    let to_string ?escape  ?styles  x =
      let buf = Buffer.create 500  in
      to_buffer ?escape ?styles buf x; Buffer.contents buf 
    ...
    and fprint_list buf (op,sep,cl,_) =
      ...
      | x::tl ->
          (Buffer.add_string buf op;
           fprint_t buf x;
           List.iter (fun x  -> Buffer.add_string buf sep; fprint_t buf x) tl;
           Buffer.add_string buf cl)
    
    and fprint_pair buf (label,_) x = fprint_t buf label; fprint_t buf x
    
    let to_buffer buf x = fprint_t buf x 
    let to_string x =
      let buf = Buffer.create 500  in to_buffer buf x; Buffer.contents buf 
    let to_formatter fmt x =
      let s = to_string x  in
      Format.fprintf fmt "%s" s; pp_print_flush fmt () 
    let to_channel oc x =
      let buf = Buffer.create 500  in
      to_buffer buf x; Buffer.output_buffer oc buf 
