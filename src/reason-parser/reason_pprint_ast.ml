  ...
  then None
    ...
    (match x.pexp_desc with
     | Pexp_extension (name,PStr ({ pstr_desc = Pstr_eval (expr,[]) }::[]))
         when name.txt <> "bs.obj" -> Some (name, expr)
     | _ -> None)
  ...
  | Some (name,expr) ->
      (match expr.pexp_desc with
       | Pexp_for _|Pexp_while _|Pexp_ifthenelse _|Pexp_function _
         |Pexp_newtype _|Pexp_try _|Pexp_match _ -> ((Some name), expr)
       | _ -> (None, x))
...
let add_extension_sugar keyword =
  function | None  -> keyword | Some str -> keyword ^ ("%" ^ str.txt) 
...
let same_ast_modulo_varification_and_extensions t1 t2 =
  let rec loop t1 t2 =
    ...
    | (Ptyp_constr ({ txt = Lident s1 },[]),Ptyp_var s2) ->
        string_equal s1 s2
    | (Ptyp_var s1,Ptyp_constr ({ txt = Lident s2 },[])) ->
        string_equal s1 s2
    ...
    | (Ptyp_arrow (label1,core_type1,core_type1'),Ptyp_arrow
       (label2,core_type2,core_type2')) ->
    ...
    | (Ptyp_object (lst1,o1),Ptyp_object (lst2,o2)) ->
        let tester (s1,attrs1,t1) (s2,attrs2,t2) =
          (string_equal s1 s2) && (loop t1 t2)  in
    ...
    | (Ptyp_variant (row_field_list1,flag1,lbl_lst_option1),Ptyp_variant
       (row_field_list2,flag2,lbl_lst_option2)) ->
...
type attributesPartition =
  ...
  uncurried: bool }[@@ocaml.doc " Kinds of attributes "]
let rec partitionAttributes ?(partDoc= false)  ?(allowUncurry= true)  attrs =
  ...
  attributesPartition) [@@ocaml.doc " Partition attributes into kinds "]
let extractStdAttrs attrs = (partitionAttributes attrs).stdAttrs 
let extract_raw_literal attrs =
  let rec loop acc =
    function
    | ({ txt = "reason.raw_literal"; loc },PStr
       ({
          pstr_desc = Pstr_eval
            ({ pexp_desc = Pexp_constant (Pconst_string (text,None ));_},_);_}::[]))::rest
        -> ((Some text), (List.rev_append acc rest))
    | [] -> (None, (List.rev acc))
    | attr::rest -> loop (attr :: acc) rest  in
  loop [] attrs 
...
let printedStringAndFixity =
  ...
  | s when
      ...
      then UnaryPlusPrefix (getPrintableUnaryIdent s)
        ...
        else if s = "!" then UnaryNotPrefix s else AlmostSimplePrefix s
  | _ -> Normal 
...
let rules =
  ...
  (TokenPrecedence,
    ((fun s  ->
        (Left, (((s.[0]) = '#') && ((s <> sharpOpEqualToken) && (s <> "#")))))));
  ...
  (TokenPrecedence,
    ((fun s  -> (Left, (((s.[0]) == '-') && (s <> fastPipeToken))))));
  (TokenPrecedence, ((fun s  -> (Left, (s = "!")))))];
  [(TokenPrecedence, ((fun s  -> (Right, (s = "::")))))];
...
let higherPrecedenceThan c1 c2 =
  ...
  | (_,None )|(None ,_) ->
      let (str1,str2) =
        ...
        | (Custom s1,Custom s2) -> (("Custom " ^ s1), ("Custom " ^ s2))  in
      raise
        (NotPossible
           ("Cannot determine precedence of two checks " ^
              (str1 ^ (" vs. " ^ str2))))
...
let type_variance =
  function | Invariant  -> "" | Covariant  -> "+" | Contravariant  -> "-" 
type construct =
  [ `cons of expression list  | `list of expression list  | `nil  | `normal 
  | `simple of Longident.t  | `tuple ]
let view_expr x =
  match x.pexp_desc with
...
let is_simple_construct =
  (function
   | `nil|`tuple|`list _|`simple _|`cons _ -> true
   | `normal -> false : construct -> bool)
...
let createFormatter () =
  let module Formatter =
    struct
      ...
      let makeList ?listConfigIfCommentsInterleaved 
        ?listConfigIfEolCommentsInterleaved  ?(break= Layout.Never)  ?(wrap=
        ("", ""))  ?(inline= (true, false))  ?(sep= Layout.NoSep)  ?(indent=
        default_indent_body)  ?(sepLeft= true)  ?(preSpace= false) 
        ?(postSpace= false)  ?(pad= (false, false))  lst =
        let config =
          {
      ...
      let makeTup ?(trailComma= true)  ?(uncurried= false)  l =
        ...
        makeList ~wrap:(lparen, ")")
          ~sep:(if trailComma then commaTrail else commaSep) ~postSpace:true
          ~break:IfNeed l
      ...
      let easyLabel ?(break= `Auto)  ?(space= false)  ?(indent=
        settings.indentAfterLabels)  labelTerm term =
        let settings =
          {
            label_break = break;
        ...
        Easy_format.Label ((labelTerm, settings), term) 
      let label ?break  ?space  ?indent  (labelTerm : Layout.t)
        (term : Layout.t) =
      ...
      let makeES6List ?wrap:((lwrap,rwrap)= ("", ""))  lst last =
        makeList ~wrap:((lwrap ^ "["), ("]" ^ rwrap)) ~break:IfNeed
          ~postSpace:true ~sep:commaTrail
          (lst @ [makeList [atom "..."; last]])
        [@@ocaml.doc " Take x,y,z and n and generate [x, y, z, ...n] "]
      ...
      let makeCommaBreakableListSurround opn cls lst =
        makeList ~break:IfNeed ~postSpace:true ~sep:(Sep ",")
          ~wrap:(opn, cls) lst
      ...
      let numLeadingSpace line =
        numLeadingSpace_ line (String.length line) 0 0 
      ...
      let inline ?(preSpace= false)  ?(postSpace= false)  labelTerm term =
        makeList [labelTerm; term] ~inline:(true, true) ~postSpace ~preSpace
          ~indent:0 ~break:Layout.Never
        
      let breakline labelTerm term =
        makeList [labelTerm; term] ~inline:(true, true) ~indent:0
          ~break:Always_rec
        ...
        | zero::one::tl ->
            ...
            let padNonOpeningLine s =
              ...
              then ""
                ...
                (String.make leftPad ' ') ^
                  (string_after s
                     (min attemptRemoveCount numLeadingSpaceForThisLine))
               in
      ...
      let rec append ?(space= false)  txt =
        function
        ...
        | Sequence (config,l) when (snd config.wrap) <> "" ->
            ...
            Sequence
              ({
                 config with
                 wrap =
                   ((fst config.wrap), ((snd config.wrap) ^ (sep ^ txt)))
               }, l)
        | Sequence (config,[]) -> Sequence (config, [atom txt])
        | Sequence (({ sep = NoSep  } as config),l)|Sequence
          (({ sep = Sep "" } as config),l) ->
            let sub =
              List.mapi
                (fun i  ->
      ...
      let rec flattenCommentAndSep ?(spaceBeforeSep= false)  ?sepStr  =
        function
        | Layout.SourceMap (loc,sub) ->
            Layout.SourceMap
              (loc, (flattenCommentAndSep ~spaceBeforeSep ?sepStr sub))
      ...
      let unbreaklayout =
        preOrderWalk
          (function
           | Layout.Sequence (listConfig,sublayouts) ->
               Layout.Sequence
                 ({ listConfig with break = Layout.Never }, sublayouts)
           | Layout.Label (formatter,left,right) ->
      ...
      let rec consolidateSeparator l =
        preOrderWalk
          (function
           | Sequence (listConfig,sublayouts) when
               (listConfig.sep != NoSep) && listConfig.sepLeft ->
               ...
               let mapSublayout i layout =
                 ...
                 | (SepFinal (sepStr,_),false )|(Sep sepStr,false ) ->
                     flattenCommentAndSep
                       ~spaceBeforeSep:(listConfig.preSpace) ~sepStr layout
                 | (SepFinal (_,finalSepStr),true ) ->
                     flattenCommentAndSep
                       ~spaceBeforeSep:(listConfig.preSpace)
                       ~sepStr:finalSepStr layout
                  in
               let layoutsWithSepAndComment =
                 List.mapi mapSublayout sublayouts  in
               let sep = Layout.NoSep  in
               let preSpace = false  in
               Sequence
                 ({ listConfig with sep; preSpace },
      ...
      let rec insertLinesAboveItems items =
        preOrderWalk
          (function
      ...
      let insertCommentIntoWhitespaceRegion comment region subLayout =
        ...
        | [] ->
            ...
            then
              ...
              if range.lnum_start = (cl.loc_start).pos_lnum
              then
                (let subLayout =
                   breakline formattedComment (insertBlankLines 1 subLayout)
                    in
                 let nextRegion =
                   WhitespaceRegion.modifyNewlines nextRegion 0  in
        ...
        | prevComment::_cs ->
            ...
            let attachedToStartRegion =
              (cl.loc_start).pos_lnum = range.lnum_start  in
            let nextRegion =
              if attachedToStartRegion
              then WhitespaceRegion.modifyNewlines nextRegion 0
              else nextRegion  in
            if Reason_location.hasSpaceBetween pcl cl
            then
              let subLayout =
                breakline formattedComment (insertBlankLines 1 subLayout)  in
              let withComment = Layout.Whitespace (nextRegion, subLayout)  in
              withComment
            else
              (let subLayout = breakline formattedComment subLayout  in
               let withComment = Layout.Whitespace (nextRegion, subLayout)
                  in
               withComment)
      ...
      let appendComment ~breakAncestors  layout comment =
        ...
        let layout =
          ...
          | Layout.Whitespace (info,sublayout) ->
              Layout.Whitespace
                (info,
                  (makeList ~break:Layout.Never ~postSpace:true
                     [sublayout; atom text]))
          | layout ->
              makeList ~break:Layout.Never ~postSpace:true
                [layout; atom text]
        ...
        if breakAncestors
        then
          makeList ~inline:(true, true) ~postSpace:false ~preSpace:true
            ~indent:0 ~break:Always_rec [layout]
        else layout 
      let rec looselyAttachComment ~breakAncestors  layout comment =
        ...
        | Layout.SourceMap (loc,sub) ->
            Layout.SourceMap
              (loc, (looselyAttachComment ~breakAncestors sub comment))
        | Layout.Whitespace (info,sub) ->
            Layout.Whitespace
              (info, (looselyAttachComment ~breakAncestors sub comment))
        | Easy e -> inline ~postSpace:true layout (formatComment comment)
        | Sequence (listConfig,subLayouts) when
            List.exists (Layout.contains_location ~location) subLayouts ->
        ...
        | Sequence (listConfig,subLayouts) ->
            let (beforeComment,afterComment) =
              Reason_syntax_util.pick_while (Layout.is_before ~location)
                subLayouts
               in
            let newSubLayout =
              ...
              | [] ->
                  Reason_syntax_util.map_first
                    (prependSingleLineComment comment) afterComment
              | hd::tl ->
                  List.rev_append ((appendComment ~breakAncestors hd comment)
                    :: tl) afterComment
               in
            Sequence (listConfig, newSubLayout)
        | Label (formatter,left,right) ->
            let (newLeft,newRight) =
              match ((Layout.get_location left), (Layout.get_location right))
              with
              | (None ,None ) ->
                  (left,
                    (looselyAttachComment ~breakAncestors right comment))
              | (_,Some loc2) when location_contains loc2 location ->
                  (left,
                    (looselyAttachComment ~breakAncestors right comment))
              | (Some loc1,_) when location_contains loc1 location ->
                  ((looselyAttachComment ~breakAncestors left comment),
                    right)
      ...
      let rec insertSingleLineComment layout comment =
        ...
        | Sequence (listConfig,subLayouts) ->
            ...
            (match afterComment with
             | [] ->
                 let break sublayout =
                   breakline sublayout (formatComment comment)  in
                 Sequence
                   (listConfig,
                     (Reason_syntax_util.map_last break beforeComment))
             | hd::tl ->
                 let afterComment =
                   ...
                   | Some loc ->
                       (Layout.SourceMap
                          (loc, (prependSingleLineComment comment hd)))
                       :: tl
                   | _ -> (prependSingleLineComment comment hd) :: tl  in
        ...
        | Label (formatter,left,right) ->
            ...
            let (newLeft,newRight) =
              ...
              | (Some loc1,Some loc2) when location_is_before location loc2
                  -> (left, (prependSingleLineComment comment right))
              | _ -> (left, (breakline right (formatComment comment)))  in
            Label (formatter, newLeft, newRight)
        [@@ocaml.doc
      ...
      let rec tryPerfectlyAttachComment layout =
        function
        | None  -> (layout, None)
      ...
      and perfectlyAttachComment comment =
        function
        | Layout.Sequence (listConfig,subLayouts) ->
            let distributeCommentIntoSubLayouts (i,processed,newComment)
              layout =
              let (layout,newComment) =
                tryPerfectlyAttachComment layout newComment  in
              ((i + 1), (layout :: processed), newComment)  in
            let (_,processed,consumed) =
              List.fold_left distributeCommentIntoSubLayouts
        ...
        | Layout.SourceMap (loc,subLayout) ->
            ...
            if
              ((loc.loc_end).Lexing.pos_lnum =
                ...
                &&
                ((commloc.loc_start).Lexing.pos_cnum =
              ...
              ((Layout.SourceMap
                  (loc,
                    (makeList ~inline:(true, true) ~break:Always
                       [unbreaklayout
                          (attachCommentToNodeRight subLayout comment)]))),
                None)
            else
              (let (layout,comment) =
                 perfectlyAttachComment comment subLayout  in
               match comment with
               | None  -> ((Layout.SourceMap (loc, layout)), None)
               | Some comment ->
                   if
                     (commloc.loc_end).Lexing.pos_cnum =
                       (loc.loc_start).Lexing.pos_cnum
                   then
                     ((Layout.SourceMap
                         (loc, (attachCommentToNodeLeft comment layout))),
                       None)
                   else
                     if
                       (commloc.loc_start).Lexing.pos_cnum =
                     ...
                     then
                       ((Layout.SourceMap
                           (loc, (attachCommentToNodeRight layout comment))),
                         None)
        ...
        | Whitespace (info,subLayout) ->
            (match perfectlyAttachComment comment subLayout with
             ...
             | (newLayout,Some c) ->
                 ((Whitespace (info, newLayout)), (Some c)))
      ...
      let rec partitionComments_ ((singleLines,endOfLines,regulars) as soFar)
        ...
        | com::tl ->
            (match Comment.category com with
             | Comment.EndOfLine  ->
                 partitionComments_
                   (singleLines, (com :: endOfLines), regulars) tl
             | Comment.SingleLine  ->
                 partitionComments_
                   ((com :: singleLines), endOfLines, regulars) tl
             | Comment.Regular  ->
                 partitionComments_
                   (singleLines, endOfLines, (com :: regulars)) tl)
          ...
          List.fold_left
            (fun (before,after)  ->
               fun comment  ->
                 let cl = Comment.location comment  in
                 let isAfter =
                   (loc.loc_end).pos_lnum < (cl.loc_start).pos_lnum  in
                 if isAfter
        ...
        let rec aux prevLoc layout i =
          ...
          | comment::cs ->
              ...
              let commentLayout =
                ...
                else formattedComment  in
              let newLayout =
                if i == 0
                then
                  makeList ~inline:(true, true) ~break:Never
                    [layout; commentLayout]
                else breakline layout commentLayout  in
              aux loc newLayout (i + 1) cs
          | [] -> layout  in
        aux loc layout 0 singleLineComments 
      let attachSingleLineComments singleLineComments =
        function
        | Layout.SourceMap (loc,subLayout) ->
            let (before,after) =
              partitionSingleLineComments loc singleLineComments  in
            let layout =
              List.fold_left insertSingleLineComment subLayout before  in
      ...
      let format_layout ?comments  ppf layout =
        let easy =
          ...
          | Some comments ->
              let (singleLines,endOfLines,regulars) =
                partitionComments comments  in
              let layout =
                List.fold_left insertRegularComment layout regulars  in
              let layout = consolidateSeparator layout  in
              let layout =
                List.fold_left insertEndOfLineComment layout endOfLines  in
              let layout = attachSingleLineComments singleLines layout  in
              let layout = insertLinesAboveItems layout  in
              let layout = Layout.to_easy_format layout  in layout
           in
        ...
        (let _ = Easy_format.Pretty.to_formatter fauxmatter easy  in
         let trimmed =
           Reason_syntax_util.processLineEndingsAndStarts
             (Buffer.contents buf)
            in
         Format.fprintf ppf "%s\n" trimmed; Format.pp_print_flush ppf ())
      ...
      let partitionFinalWrapping listTester wrapFinalItemSetting x =
        ...
        | ([],_) ->
            raise
              (NotPossible "shouldnt be partitioning 0 label attachments")
      ...
      let makeLetSequence letItems =
        makeList ~break:Always_rec ~inline:(true, false) ~wrap:("{", "}")
          ~postSpace:true ~sep:(SepFinal (";", ";")) letItems
        
      let makeLetSequenceSingleLine letItems =
        makeList ~break:IfNeed ~inline:(true, false) ~wrap:("{", "}")
          ~preSpace:true ~postSpace:true ~sep:(Sep ";") letItems
        
      let makeUnguardedLetSequence letItems =
        makeList ~break:Always_rec ~inline:(true, true) ~wrap:("", "")
          ~indent:0 ~postSpace:true ~sep:(SepFinal (";", ";")) letItems
        
      let formatSimpleAttributed x y =
        makeList ~wrap:("(", ")") ~break:IfNeed ~indent:0 ~postSpace:true
          (List.concat [y; [x]])
        
      let formatAttributed ?(labelBreak= `Auto)  x y =
        label ~break:labelBreak ~indent:0 ~space:true
          (makeList ~inline:(true, true) ~postSpace:true y) x
      ...
      let formatCoerce expr optType coerced =
        match optType with
        | None  ->
            label ~space:true (makeList ~postSpace:true [expr; atom ":>"])
              coerced
        | Some typ ->
            label ~space:true
              (makeList ~postSpace:true
                 [formatTypeConstraint expr typ; atom ":>"]) coerced
      ...
      let formatIndentedApplication headApplicationItem argApplicationItems =
        makeList ~inline:(true, true) ~postSpace:true ~break:IfNeed
          [label ~space:true headApplicationItem
             (makeAppList argApplicationItems)]
        
      let formatAttachmentApplication finalWrapping
        (attachTo : (bool * Layout.t) option) (appTermItems,loc) =
        ...
        | Some (attachedList,wrappedListy) ->
            (match (attachedList, attachTo) with
             ...
             | ([],None ) -> source_map ?loc wrappedListy
             | (hd::tl,Some (useSpace,toThis)) ->
      ...
      let applicationFinalWrapping x =
        partitionFinalWrapping isSequencey settings.funcApplicationLabelStyle
          x
      ...
      let constant_string ppf s = Format.fprintf ppf "%S" s 
      let tyvar ppf str = Format.fprintf ppf "'%s" str 
      let constant ?raw_literal  ?(parens= true)  ppf =
        function
        ...
        | Pconst_string (i,None ) ->
            (match raw_literal with
             ...
             | None  ->
                 Format.fprintf ppf "\"%s\""
                   (Reason_syntax_util.escape_string i))
        ...
        | Pconst_integer (i,Some m) ->
            paren (parens && ((i.[0]) = '-'))
              (fun ppf  -> fun (i,m)  -> Format.fprintf ppf "%s%c" i m) ppf
              (i, m)
        | Pconst_float (i,None ) ->
            paren (parens && ((i.[0]) = '-'))
              (fun ppf  -> Format.fprintf ppf "%s") ppf i
      ...
      let is_punned_labelled_expression e lbl =
        ...
        | _ -> false 
      let is_punned_labelled_pattern p lbl =
        match p.ppat_desc with
        | Ppat_constraint
            ({ ppat_desc = Ppat_var { txt;_}; ppat_attributes = _::_ },_) ->
            false
        | Ppat_constraint ({ ppat_desc = Ppat_var { txt;_};_},_)|Ppat_var
          { txt;_} -> txt = lbl
        | _ -> false 
      ...
      let recordRowIsPunned pld =
        ...
        | { ptyp_desc = Ptyp_constr ({ txt;_},[]);_} when
      ...
      let isJSXComponent expr =
        ...
        | { pexp_desc = Pexp_apply ({ pexp_desc = Pexp_ident _ },args);
          ...
          |{
             pexp_desc = Pexp_apply
               ({ pexp_desc = Pexp_letmodule (_,_,_) },args);
             pexp_attributes }
            ->
            ...
            let rec hasSingleNonLabelledUnitAndIsAtTheEnd l =
              ...
              | _::rest -> hasSingleNonLabelledUnitAndIsAtTheEnd rest  in
            if
              (jsxAttrs != []) &&
        ...
        | _ -> false 
      let isSingleArgParenApplication =
        function
        ...
        | { pexp_attributes = []; pexp_desc = Pexp_extension (s,_) }::[] when
            s.txt = "bs.obj" -> true
        | ({ pexp_attributes = []; pexp_desc } as exp)::[] when
            is_simple_list_expr exp -> true
        | _ -> false 
      let isSingleArgParenPattern =
        function
      ...
      let formatComputedInfixChain infixChainList =
        let layout_of_group group currentToken =
          ...
          then makeList ~inline:(true, true) ~sep:(Sep " ") group
          else
              ...
              (let hd = List.hd group  in
               let tl =
                 makeList ~inline:(true, true) ~sep:(Sep " ") (List.tl group)
                  in
               makeList ~inline:(true, true) ~sep:(Sep " ") ~break:IfNeed
                 [hd; tl])
            else
              if (currentToken.[0]) = '#'
              then
                ...
                label ~break:`Never ~space:true (atom currentToken)
                  (List.nth group 1)
           in
          ...
          | x::xs ->
              (match x with
               | InfixToken t ->
                   ...
                   then
                     let groupNode =
                       makeList ~inline:(true, true) ~sep:(Sep " ")
                         ((print [] group currentToken []) @ [atom t])
                        in
                     let children =
                       makeList ~inline:(true, true) ~preSpace:true
                         ~break:IfNeed (print [] [] t xs)
                        in
                     print (acc @ [label ~space:true groupNode children]) []
                       t []
                   else
                     if t = "@@"
                     then
                       ...
                       if List.mem t equalityOperators
                       then
                         print acc
                           ((print [] group currentToken []) @ [atom t]) t xs
                       else
                         if requireNoSpaceFor t
                         then
                           (if
                              (currentToken = "") ||
                                (requireNoSpaceFor currentToken)
                            then print acc (group @ [atom t]) t xs
                              ...
                              (match List.rev group with
                               | hd::tl ->
                                   let acc =
                                     acc @
                                       [layout_of_group (List.rev tl)
                                          currentToken]
                                      in
                                   print acc [hd; atom t] t xs
                               | [] -> print acc (group @ [atom t]) t xs))
                         else
                           print (acc @ [layout_of_group group currentToken])
                             [atom t] t xs
               | Layout layoutNode ->
      ...
      let groupAndPrint ~xf  ~getLoc  ~comments  items =
        let rec group prevLoc curr acc =
          function
          | x::xs ->
              let item = xf x  in
              let loc = getLoc x  in
                ...
                (fun i  ->
                   fun group  ->
                     ...
                     | curr::xs ->
                         ...
                         let region =
                           WhitespaceRegion.make ~range ~newlines ()  in
      ...
      let printer =
        object (self : 'self)
          ...
          method longident =
            ...
            | Lapply (y,s) ->
                makeList
                  [self#longident y; atom "("; self#longident s; atom ")"]
          ...
          method non_arrowed_core_type x =
            self#non_arrowed_non_simple_core_type x
          method core_type2 x =
            let { stdAttrs; uncurried } =
              partitionAttributes x.ptyp_attributes  in
            let uncurried =
              uncurried ||
                (try Hashtbl.find uncurriedTable x.ptyp_loc
            ...
            then
              ...
              (let x =
                 if uncurried then { x with ptyp_attributes = [] } else x  in
               match x.ptyp_desc with
               | Ptyp_arrow (l,ct1,ct2) ->
                   let rec allArrowSegments ?(uncurried= false)  acc =
                     function
                     ...
                     | rhs ->
                         ...
                         let is_tuple typ =
                           ...
                           | _ -> false  in
                         (match acc with
                          | (Nolabel ,lhs,uncurried)::[] when
                              not (is_tuple lhs) ->
                              let t = self#non_arrowed_simple_core_type lhs
                                 in
                              let lhs =
                                if uncurried
                                then
                                  makeList ~wrap:("(. ", ")") ~postSpace:true
                                    [t]
                                else t  in
                              (lhs, rhs)
                          | acc ->
                              let params =
                                List.rev_map self#type_with_label acc  in
                              ((makeCommaBreakableListSurround "(" ")" params),
                                rhs))
                      in
                   let (lhs,rhs) = allArrowSegments ~uncurried [] x  in
                   let normalized =
                     makeList ~preSpace:true ~postSpace:true
                       ~inline:(true, true) ~break:IfNeed ~sep:(Sep "=>")
                       [lhs; rhs]
                      in
                   ...
                   let poly =
                     ...
                     | sl ->
                         makeList ~break:IfNeed ~postSpace:true
                           [makeList
                              [makeList ~postSpace:true
                                 (List.map (fun x  -> self#tyvar x) sl);
                              atom "."];
                           ct]
                      in
              ...
              (match x.ptyp_desc with
               | Ptyp_alias (ct,s) ->
                   source_map ~loc:(x.ptyp_loc)
                     (label ~space:true (self#core_type ct)
          ...
          method type_with_label (lbl,c,uncurried) =
            ...
            let t =
              ...
              | Labelled lbl ->
                  makeList ~sep:(Sep " ")
                    [atom (namedArgSym ^ (lbl ^ ":")); typ]
              | Optional lbl ->
                  makeList ~sep:(Sep " ")
                    [atom (namedArgSym ^ (lbl ^ ":")); label typ (atom "=?")]
               in
          ...
          method formatOneTypeDef prepend name assignToken
            ({ ptype_params; ptype_kind; ptype_manifest; ptype_loc } as td) =
            let (equalInitiatedSegments,constraints) =
              self#type_declaration_binding_segments td  in
            ...
            let labelWithParams =
              ...
              | [] -> binding
              | l -> label binding (makeTup l)  in
            let everythingButConstraints =
              let nameParamsEquals =
                makeList ~postSpace:true [labelWithParams; assignToken]  in
              match equalInitiatedSegments with
              | [] -> labelWithParams
              | hd::hd2::hd3::tl ->
                  raise (NotPossible "More than two type segments.")
              | hd::[] ->
                  formatAttachmentApplication typeApplicationFinalWrapping
                    (Some (true, nameParamsEquals)) (hd, None)
              | hd::hd2::[] ->
                  let first =
                    makeList ~postSpace:true ~break:IfNeed
                      ~inline:(true, true) (hd @ [atom "="])
                     in
                  let second =
            ...
            let everything =
              ...
              | hd::tl ->
                  makeList ~break:IfNeed ~postSpace:true ~indent:0
                    ~inline:(true, true) (everythingButConstraints :: hd ::
                    tl)
               in
          ...
          method formatOneTypeExt prepend name assignToken te =
            ...
            let equalInitiatedSegments =
              let segments =
                List.map self#type_extension_binding_segments
                  te.ptyext_constructors
                 in
              let privatized_segments = privatize te.ptyext_private segments
                 in
              [makeList ~break:Always_rec ~postSpace:true
                 ~inline:(true, true) privatized_segments]
               in
            let formattedTypeParams =
              List.map self#type_param te.ptyext_params  in
            let binding = makeList ~postSpace:true [prepend; name]  in
            let labelWithParams =
              match formattedTypeParams with
              | [] -> binding
          ...
          method type_extension_binding_segments
            ...
            let add_bar name attrs args =
              let lbl =
                match args with
                | None  -> name
                | Some args -> label name args  in
              if attrs <> []
              then
                label ~space:true
                  (makeList ~postSpace:true
                     [atom "|";
                     makeList ~postSpace:true ~break:Layout.IfNeed
                       ~inline:(true, true) (self#attributes attrs)]) lbl
              else makeList ~postSpace:true [atom "|"; lbl]  in
            let sourceMappedName = atom ~loc:(pext_name.loc) pext_name.txt
               in
            let resolved =
              ...
              | Pext_decl (ctor_args,gadt) ->
                  let formattedArgs =
                    ...
                    | Pcstr_tuple args ->
                        [makeTup
                           (List.map self#non_arrowed_non_simple_core_type
                              args)]
                    | Pcstr_record r -> [self#record_declaration r]  in
                  let formattedGadt =
                    match gadt with
                    | None  -> None
                    | Some x ->
            ...
            let everything =
              ...
              | (ctorArgs,None ) ->
                  add_bar sourceMappedName pext_attributes
          ...
          method type_def_list (rf,l) =
            let formatOneTypeDefStandard prepend td =
              let itm =
                self#formatOneTypeDef prepend
                  (atom ~loc:((td.ptype_name).loc) (td.ptype_name).txt)
                  (atom "=") td
                 in
              let { stdAttrs; docAttrs } =
                partitionAttributes ~partDoc:true td.ptype_attributes  in
              let layout = self#attach_std_item_attrs stdAttrs itm  in
              self#attachDocAttrsToLayout ~stdAttrs ~docAttrs
                ~loc:(td.ptype_loc) ~layout ()
               in
            match l with
            ...
            | hd::tl ->
                ...
                (match tl with
                 | [] -> first
                 | tlhd::tltl ->
                     makeList ~indent:0 ~inline:(true, true)
                       ~break:Always_rec (first ::
                       (List.map (formatOneTypeDefStandard (atom "and"))
                          (tlhd :: tltl))))
          method type_variant_leaf ?opt_ampersand:(a= false) 
            ?polymorphic:(p= false)  = self#type_variant_leaf1 a p true
          method type_variant_leaf_nobar ?opt_ampersand:(a= false) 
            ?polymorphic:(p= false)  = self#type_variant_leaf1 a p false
          method type_variant_leaf1 opt_ampersand polymorphic print_bar x =
            ...
            let ampersand_helper i arg =
              ...
              then ct
              else label (atom "&") ct  in
            let args =
              match pcd_args with
              | Pcstr_record r -> [self#record_declaration r]
              | Pcstr_tuple [] -> []
              | Pcstr_tuple l when polymorphic ->
                  List.mapi ampersand_helper l
              | Pcstr_tuple l -> [makeTup (List.map self#core_type l)]  in
            let gadtRes =
              match pcd_res with
              | None  -> None
              | Some x ->
                  Some (formatJustTheTypeConstraint (self#core_type x))
               in
            let normalize lst =
              match lst with
              | [] -> raise (NotPossible "should not be called")
            ...
            let add_bar_2 ?gadt  name args =
              ...
              let fullLbl =
                match gadt with
                | Some g ->
                    makeList ~inline:(true, true) ~break:IfNeed [lbl; g]
                | None  -> lbl  in
              add_bar fullLbl  in
            let prefix = if polymorphic then "`" else ""  in
            let sourceMappedName =
              atom ~loc:(pcd_name.loc) (prefix ^ pcd_name.txt)  in
            let sourceMappedNameWithAttributes =
              match stdAttrs with
              | [] -> sourceMappedName
              | stdAttrs ->
                  formatAttributed sourceMappedName
                    (self#attributes stdAttrs)
               in
            let constructorName =
              makeList ~postSpace:true [sourceMappedNameWithAttributes]  in
            let everything =
              match (args, gadtRes) with
              | ([],None ) -> add_bar sourceMappedNameWithAttributes
              | ([],Some gadt) ->
                  add_bar_2 sourceMappedNameWithAttributes gadt
          ...
          method record_declaration ?assumeRecordLoc  lbls =
            let recordRow i pld =
              ...
              let name =
                ...
                else [atom (pld.pld_name).txt; atom ":"]  in
              let name = source_map ~loc:((pld.pld_name).loc) (makeList name)
                 in
              let withMutable =
              ...
              let recordRow =
                ...
                | attrs ->
                    let { stdAttrs; docAttrs } =
                      partitionAttributes ~partDoc:true attrs  in
                    let stdAttrsLayout =
                      makeList ~inline:(true, true) ~postSpace:true
                        (self#attributes stdAttrs)
                       in
                    let docAttrsLayout =
                      makeList ~inline:(true, true)
                        (self#attributes docAttrs)
                       in
                    let children =
          ...
          method type_declaration_binding_segments x =
            ...
            let estimateRecordOpenBracePoint () =
              ...
              | hd::tl ->
                  ((fst
                      (List.nth x.ptype_params
                         ((List.length x.ptype_params) - 1))).ptyp_loc).loc_end
               in
            let equalInitiatedSegments =
              match ((x.ptype_kind), (x.ptype_private), (x.ptype_manifest))
              with
              ...
              | (Ptype_record lst,scope,None ) ->
                  ...
                  [privatize scope
                     [self#record_declaration ~assumeRecordLoc lst]]
              ...
              | (_,_,_) ->
                  raise
                    (NotPossible "Encountered impossible type specification")
               in
            ...
            then
              ...
              (let result =
                 ...
                 | Ptyp_package (lid,cstrs) ->
                     self#typ_package ~protect:true ~mod_prefix:true lid
                       cstrs
                 | Ptyp_var s ->
                     ensureSingleTokenSticksToLabel (self#tyvar s)
                 ...
                 | Ptyp_constr (li,l) ->
                     (match l with
                      | { ptyp_desc = Ptyp_object ((_::_ as l),o) }::[] when
                          isJsDotTLongIdent li.txt ->
                          self#unparseObject ~withStringKeys:true l o
                      | { ptyp_desc = Ptyp_object (l,o) }::[] when
                          not (isJsDotTLongIdent li.txt) ->
                          label (self#longident_loc li)
                            (self#unparseObject ~wrap:("(", ")") l o)
                      | {
                          ptyp_desc = Ptyp_constr
                            (lii,{ ptyp_desc = Ptyp_object ((_::_ as ll),o) }::[])
                          }::[] when isJsDotTLongIdent lii.txt ->
                          label (self#longident_loc li)
                            (self#unparseObject ~withStringKeys:true
                               ~wrap:("(", ")") ll o)
                      | _ ->
                          label (self#longident_loc li)
                            (makeTup
                 ...
                 | Ptyp_variant (l,closed,low) ->
                     ...
                     let variant_helper i rf =
                       ...
                       | Rtag (label,attrs,opt_ampersand,ctl) ->
                           let pcd_name = { txt = label; loc = pcd_loc }  in
                           let pcd_args = Pcstr_tuple ctl  in
                           let all_attrs =
                             List.concat [pcd_attributes; attrs]  in
                           self#type_variant_leaf ~opt_ampersand
                             ~polymorphic:true
                             {
                               pcd_name;
                               pcd_args;
          ...
          method pattern_list_split_cons acc =
            ...
            | {
                ppat_desc = Ppat_construct
                  ({ txt = Lident "::"; loc = consLoc },Some
                   { ppat_desc = Ppat_tuple (pat1::pat2::[]) })
          ...
          method or_pattern p1 p2 =
            ...
            let (left,right) =
              ...
              | _ -> (p1_raw, p2_raw)  in
            makeList ~break:IfNeed ~inline:(true, true) ~sep:(Sep "|")
              ~postSpace:true ~preSpace:true [left; right]
          method pattern_without_or x =
            ...
            | Ppat_alias (p,s) ->
                ...
                let pattern_with_precedence =
                  match p.ppat_desc with
                  | Ppat_or (p1,p2) ->
                      formatPrecedence (self#or_pattern p1 p2)
                  | _ -> raw_pattern  in
                label ~space:true
            ...
            | Ppat_lazy p ->
                label ~space:true (atom "lazy") (self#simple_pattern p)
            | Ppat_construct (({ txt } as li),po) when
                not (txt = (Lident "::")) ->
                let formattedConstruction =
                  match po with
                  | Some pattern ->
                      let arityIsClear = isArityClear arityAttrs  in
                      self#constructor_pattern ~arityIsClear
                        (self#longident_loc li) pattern
                  | None  -> self#longident_loc li  in
                source_map ~loc:(x.ppat_loc) formattedConstruction
            | _ -> self#simple_pattern x
          method pattern x =
            let { arityAttrs; stdAttrs } =
              partitionAttributes x.ppat_attributes  in
            if stdAttrs <> []
            then
              formatAttributed
          ...
          method patternList ?(wrap= ("", ""))  pat =
            ...
            | { ppat_desc = Ppat_construct ({ txt = Lident "[]" },_) } ->
                ...
                makeList pat_list ~break:Layout.IfNeed ~sep:commaTrail
                  ~postSpace:true ~wrap:((lwrap ^ "["), ("]" ^ rwrap))
            | _ -> makeES6List pat_list (self#pattern pat_last) ~wrap
          method typ_package ?(protect= false)  ?(mod_prefix= true)  lid
            cstrs =
            let packageIdent =
              ...
              then
                makeList ~postSpace:true ~wrap:("(", ")")
                  [if mod_prefix then atom "module" else atom "";
                  packageIdent]
              else
                if mod_prefix
                     ...
                     (List.map
                        (fun (s,ct)  ->
                           label ~space:true
                             ...
                             (self#core_type ct)) cstrs))
          method constrained_pattern x =
            match x.ppat_desc with
            | Ppat_constraint (p,ct) ->
                let (pat,typ) =
                  ...
                  | ({ ppat_desc = Ppat_unpack unpack },{
                      ...
                      ->
                      ((makeList ~postSpace:true
                          [atom "module"; atom unpack.txt]),
                        (self#typ_package ~mod_prefix:false lid cstrs))
                  | _ -> ((self#pattern p), (self#core_type ct))  in
          ...
          method simple_pattern x =
            ...
            then
              ...
              (let itm =
                 ...
                 | Ppat_unpack s ->
                     makeList ~wrap:("(", ")") ~break:IfNeed ~postSpace:true
                       [atom "module"; atom s.txt]
                 ...
                 | Ppat_constant c ->
                     let (raw_literal,_) =
                       extract_raw_literal x.ppat_attributes  in
                 ...
                 | Ppat_constraint (p,ct) ->
                     formatPrecedence
                       (formatTypeConstraint (self#pattern p)
                          (self#core_type ct))
                 | Ppat_lazy p ->
                     formatPrecedence
                       (label ~space:true (atom "lazy")
                          (self#simple_pattern p))
                 | Ppat_extension e -> self#extension e
                 | Ppat_exception p ->
                     makeList ~postSpace:true
                       [atom "exception"; self#simple_pattern p]
                 | _ -> formatPrecedence (self#pattern x)  in
          ...
          method label_exp lbl opt pat =
            ...
            let param =
              match lbl with
              | Nolabel  -> term
              | Labelled lbl|Optional lbl when
                  is_punned_labelled_pattern pat lbl ->
            ...
            | (Some o,_) ->
                makeList
                  [param;
                  atom "=";
                  self#unparseConstraintExpr ~ensureExpr:true o]
          method access op cls e1 e2 = makeList [e1; atom op; e2; atom cls]
          method simple_get_application x =
            let { stdAttrs; jsxAttrs } =
              partitionAttributes x.pexp_attributes  in
            match ((x.pexp_desc), stdAttrs, jsxAttrs) with
            | (_,attrHd::attrTl,[]) -> None
            | (Pexp_apply ({ pexp_desc = Pexp_ident loc },l),[],_jsx::_) ->
                ...
                then
                  let moduleNameList =
                    List.rev (List.tl (List.rev (Longident.flatten loc.txt)))
                     in
                  (if moduleNameList != []
                   then
                     (if (Longident.last loc.txt) = "createElement"
                      then
                        Some
                          (self#formatJSXComponent
                             (String.concat "." moduleNameList) l)
                      else None)
                   else
                     Some
                       (self#formatJSXComponent (Longident.last loc.txt) l))
                else None
            | (Pexp_apply
               ({
                  pexp_desc = Pexp_letmodule
                    (_,({ pmod_desc = Pmod_apply (m1,m2) } as app),{
                                                                    pexp_desc
                                                                    =
                                                                    Pexp_ident
                                                                    loc })
                ...
                ->
                let rec extract_apps args =
                  function
                  | {
                      pmod_desc = Pmod_apply
                        (m1,{ pmod_desc = Pmod_ident loc })
                      } ->
                      let arg = String.concat "." (Longident.flatten loc.txt)
                         in
                      extract_apps (arg :: args) m1
                  | { pmod_desc = Pmod_ident loc } ->
                      (String.concat "." (Longident.flatten loc.txt)) :: args
                  | _ ->
                      failwith
                ...
                if
                  hasLabelledChildrenLiteral &&
                    (hasSingleNonLabelledUnitAndIsAtTheEnd l)
                then
                  (if (List.length (Longident.flatten loc.txt)) > 1
                   then
                     (if (Longident.last loc.txt) = "createElement"
                      then
                        ...
                        | ftor::args ->
                            ...
                            Some
                              (self#formatJSXComponent
                                 ~closeComponentName:ftor applied l)
                        | _ -> None
                      else None)
                   else
                     Some
               ...
               | Pexp_apply
                   ...
                   let lhs =
                     self#unparseResolvedRule
                      ...
                      in
                   Some
                     ((self#access ".[" "]" lhs (self#unparseExpr e2)), e3)
               | Pexp_apply
                   ({
                      pexp_desc = Pexp_ident
                    ...
                    | "Genarray" ->
                        (match label_exprs with
                         | (_,a)::(_,{ pexp_desc = Pexp_array ls })::
                             (_,c)::[] ->
                             let formattedList = List.map self#unparseExpr ls
                                in
                             let lhs =
                               makeList
                                 [self#simple_enough_to_be_lhs_dot_send a;
                                 atom "."]
                                in
                             let rhs =
                               makeList ~break:IfNeed ~postSpace:true
                                 ~sep:commaSep ~wrap:("{", "}") formattedList
                                in
                         ...
                         | (_,a)::rest ->
                             (match List.rev rest with
                              | (_,v)::rest ->
                                  ...
                                  let formattedList =
                                    List.map self#unparseExpr args  in
                                  let lhs =
                                    makeList
          ...
          method ensureContainingRule ~withPrecedence  ~reducesAfterRight  ()
            ...
            | SpecificInfixPrecedence
                ({ reducePrecedence; shiftPrecedence },rightRecurse) ->
                if higherPrecedenceThan shiftPrecedence withPrecedence
                then rightRecurse
                else
                  if higherPrecedenceThan withPrecedence shiftPrecedence
                  then
                    LayoutNode
                      (formatPrecedence ~loc:(reducesAfterRight.pexp_loc)
                         (self#unparseResolvedRule rightRecurse))
                  else
                    if isRightAssociative ~prec:withPrecedence
                    then rightRecurse
                    else
                      LayoutNode
                        (formatPrecedence ~loc:(reducesAfterRight.pexp_loc)
            ...
            | FunctionApplication itms ->
                let funApplExpr =
                  formatAttachmentApplication applicationFinalWrapping None
                   ...
                   in
                if
                  higherPrecedenceThan withPrecedence
                    (Custom "prec_functionAppl")
                then
                  LayoutNode
                    (formatPrecedence ~loc:(reducesAfterRight.pexp_loc)
                       funApplExpr)
            ...
            | PotentiallyLowPrecedence itm ->
                LayoutNode
                  (formatPrecedence ~loc:(reducesAfterRight.pexp_loc) itm)
            | Simple itm -> LayoutNode itm
          method ensureExpression ~reducesOnToken  expr =
            match self#unparseExprRecurse expr with
            | SpecificInfixPrecedence
                ({ reducePrecedence; shiftPrecedence },leftRecurse) ->
                if higherPrecedenceThan reducePrecedence reducesOnToken
                then leftRecurse
                else
                  if higherPrecedenceThan reducesOnToken reducePrecedence
                  then
                    LayoutNode
                      (formatPrecedence ~loc:(expr.pexp_loc)
                         (self#unparseResolvedRule leftRecurse))
                  else
                    if isLeftAssociative ~prec:reducesOnToken
                    then leftRecurse
                    else
                      LayoutNode
                        (formatPrecedence ~loc:(expr.pexp_loc)
          ...
          method unparseConstraintExpr ?(ensureExpr= false)  e =
            let itm =
              ...
              | { pexp_attributes = []; pexp_desc = Pexp_constraint (x,ct) }
                  ...
                  let children =
                    [x; label ~space:true (atom ":") (self#core_type ct)]  in
                  if ensureExpr
              ...
              | { pexp_attributes; pexp_desc = Pexp_constant c } ->
                  let (raw_literal,pexp_attributes) =
                    extract_raw_literal pexp_attributes  in
                  let constant =
                    self#constant ?raw_literal ~parens:ensureExpr c  in
                   ...
                   | attrs ->
                       let formattedAttrs =
                         makeSpacedBreakableInlineList
                          ...
                          in
                       makeSpacedBreakableInlineList
                         [formattedAttrs; constant])
              | x -> self#unparseExpr x  in
            source_map ~loc:(e.pexp_loc) itm
          method simplifyUnparseExpr ?(wrap= ("(", ")"))  x =
            match self#unparseExprRecurse x with
            | SpecificInfixPrecedence
                ({ reducePrecedence; shiftPrecedence },itm) ->
                formatPrecedence ~wrap ~loc:(x.pexp_loc)
            ...
            | FunctionApplication itms ->
                formatPrecedence ~wrap ~loc:(x.pexp_loc)
                  (formatAttachmentApplication applicationFinalWrapping None
                     (itms, (Some (x.pexp_loc))))
            | PotentiallyLowPrecedence itm ->
                formatPrecedence ~wrap ~loc:(x.pexp_loc) itm
            | Simple itm -> itm
          ...
          method unparseExprApplicationItems x =
            match self#unparseExprRecurse x with
            | SpecificInfixPrecedence
                ({ reducePrecedence; shiftPrecedence },wrappedRule) ->
                    ...
                    let formatted =
                      ...
                      then
                        self#ensureExpression
                          ~reducesOnToken:(Token fastPipeToken) expr
                      else
                        (match expr with
                         | expr when
                             Reason_heuristics.isUnderscoreApplication expr
                       ...
                       in
                    self#unparseResolvedRule formatted  in
                  let parens =
                    ...
                    | args ->
                        let fakeApplExp =
                          let loc_end =
                            ...
                            | _ -> (exp.pexp_loc).loc_end  in
                          {
                            exp with
                            pexp_loc = { (exp.pexp_loc) with loc_end }
                          }  in
                        makeList
                          (self#formatFunAppl ~jsxAttrs:[] ~args ~funExpr:exp
                             ~applicationExpr:fakeApplExp ~uncurried ())
                     in
                  if parens then formatPrecedence layout else layout 
              end in
              let rec flatten ?(uncurried= false)  acc =
                function
                | {
                    pexp_desc = Pexp_apply
                      ({
                ...
                | { pexp_attributes;
                    ...
                    (match pexp_attributes with
                     | ({ txt = "bs" },PStr [])::[] ->
                         flatten ((Fastpipetree.ExpU arg2) :: args :: acc)
                           arg1
                     | [] ->
                         if uncurried
                         then
                           ...
                           flatten ((Fastpipetree.Exp arg2) :: args :: acc)
                             arg1
                     | _ -> (Fastpipetree.Exp e) :: acc)
                | { pexp_desc = Pexp_ident { txt = Longident.Lident "|." } }
                    -> acc
                | arg -> (Fastpipetree.Exp arg) :: acc  in
              let rec parse acc =
                function
                | (Fastpipetree.Exp e)::(Fastpipetree.Args args)::xs ->
                    parse
                      ((let open Fastpipetree in
                          { exp = e; args; uncurried = false }) :: acc) xs
                | (Fastpipetree.ExpU e)::(Fastpipetree.Args args)::xs ->
                    parse
                      ((let open Fastpipetree in
                          { exp = e; args; uncurried = true }) :: acc) xs
                | (Fastpipetree.Exp e)::xs ->
                    parse
                      ((let open Fastpipetree in
                          { exp = e; args = []; uncurried = false }) :: acc)
                      xs
                | _ -> List.rev acc  in
              let uncurried =
                try Hashtbl.find uncurriedTable e.pexp_loc
                with | Not_found  -> false  in
              let (flatNodes : Fastpipetree.flatT) = flatten ~uncurried [] e
                 in
            ...
            let process_application expr =
              let process_arg (l,e) =
                ...
                | Pexp_ident ({ txt = Lident "__x" } as id) ->
                    let pexp_desc = Pexp_ident { id with txt = (Lident "_") }
                       in
                    (l, { e with pexp_desc })
                | _ -> (l, e)  in
              match expr.pexp_desc with
              | Pexp_apply (e_fun,args) ->
                  let pexp_desc =
                    Pexp_apply (e_fun, (List.map process_arg args))  in
                  { expr with pexp_desc }
              | _ -> expr  in
            match x.pexp_desc with
            | Pexp_fun
            ...
            let x =
              {
                x with
                pexp_attributes =
                  (literalAttrs @ (arityAttrs @ (stdAttrs @ jsxAttrs)))
              }  in
            if stdAttrs <> []
            then
              let withoutVisibleAttrs =
                { x with pexp_attributes = (arityAttrs @ jsxAttrs) }  in
              ...
              let itms =
                ...
                | Simple itm -> [itm]  in
              FunctionApplication
                [makeList ~break:IfNeed ~inline:(true, true) ~indent:0
                   ~postSpace:true (List.concat [attributesAsList; itms])]
            else
                   ...
                   (match x.pexp_desc with
                    | Pexp_apply (e,ls) ->
                        let ls =
                          List.map
                            (fun (l,expr)  ->
                               (l,
                                 (self#process_underscore_application expr)))
                            ls
                           in
                        (match (e, ls) with
                              ...
                              pexp_desc = Pexp_ident
                                {
                                  txt = Ldot
                                    (Ldot
                                     (Lident "Bigarray","Genarray"),"get")
                                  }
                              },(_,e1)::(_,({ pexp_desc = Pexp_array ls } as
                                              e2))::[])
                             ->
                             if Reason_heuristics.isUnderscoreIdent e1
                             then
                               ...
                               (let formattedList =
                                  List.map self#unparseExpr ls  in
                                let lhs =
                                  makeList
                                    [self#simple_enough_to_be_lhs_dot_send e1;
                                    atom "."]
                                   in
                                let rhs =
                                  makeList ~break:IfNeed ~postSpace:true
                             ...
                             then
                               ...
                               let v =
                                 makeList ~postSpace:true
                                   ...
                                   ((atom "_") ::
                                   (List.map
                                      (fun (_,e)  -> self#unparseExpr e) rest))
                                  in
                               Simple (label k v)
                             else
                               (let formattedList =
                                  List.map self#unparseExpr
                                    (List.map snd rest)
                                   in
                                let lhs =
                                  makeList
                         ...
                         | _ ->
                             (match self#sugar_set_expr_parts x with
                              ...
                              | None  ->
                                  (match ((printedStringAndFixityExpr e), ls)
                                   with
                                   | (AlmostSimplePrefix
                                      prefixStr,(Nolabel ,rightExpr)::[]) ->
                                       let forceSpace =
                                         match rightExpr.pexp_desc with
                                         | Pexp_apply (ee,lsls) ->
                                             (match printedStringAndFixityExpr
                                                      ee
                                              with
                                              | AlmostSimplePrefix _ -> true
                                              | _ -> false)
                                       ...
                                       SpecificInfixPrecedence
                                         ({
                                           ...
                                           (LayoutNode
                                              (label ~space:forceSpace
                                                 (atom prefixStr) rightItm)))
                                   | (UnaryPostfix
                                      postfixStr,(Nolabel ,leftExpr)::[]) ->
                                       let forceSpace =
                                       ...
                                       let leftItm =
                                         ...
                                         | Pexp_apply (e,_) ->
                                             (match printedStringAndFixityExpr
                                                      e
                                              with
                                              | Infix printedIdent when
                                                  requireNoSpaceFor
                                                    printedIdent
                                                  ->
                                                  self#unparseExpr leftExpr
                                              | _ ->
                                                  self#simplifyUnparseExpr
                                                    leftExpr)
                                         | Pexp_field _ ->
                                             self#unparseExpr leftExpr
                                         | _ ->
                                          ...
                                          in
                                       Simple
                                         (label ~space:forceSpace leftItm
                                            (atom postfixStr))
                                   | (Infix
                                      printedIdent,(Nolabel ,leftExpr)::
                                      (Nolabel ,rightExpr)::[]) ->
                                              ...
                                              | Pexp_apply (e,_) ->
                                                  (match printedStringAndFixityExpr
                                                   ...
                                                   | UnaryPostfix "^" when
                                                       ...
                                                       LayoutNode
                                                         (formatPrecedence
                                                            ~loc:(leftExpr.pexp_loc)
                                                            x)
                                                   | _ -> leftItm)
                                              | _ -> leftItm)
                                         | InfixTree _ -> leftItm  in
                                       let infixTree =
                                         InfixTree
                                           (printedIdent, formattedLeftItm,
                                             rightItm)
                                          in
                                       SpecificInfixPrecedence
                                         ({
                                   ...
                                   | (UnaryPlusPrefix
                                      printedIdent,(Nolabel ,rightExpr)::[])
                                       ->
                                   ...
                                   | (_,_) ->
                                       let uncurried =
                                         try
                                           Hashtbl.find uncurriedTable
                                             x.pexp_loc
                                         with | Not_found  -> false  in
                                       FunctionApplication
                                         (self#formatFunAppl ~uncurried
                                            ~jsxAttrs ~args:ls
                                            ~applicationExpr:x ~funExpr:e ()))))
                    | Pexp_field (e,li) ->
                        let prec = Token "."  in
                        let leftItm =
                          self#unparseResolvedRule
                           ...
                           in
                        let { stdAttrs } =
                          partitionAttributes e.pexp_attributes  in
                        let formattedLeftItm =
                          if stdAttrs = []
                          then leftItm
                          else formatPrecedence ~loc:(e.pexp_loc) leftItm  in
                        let layout =
                          label (makeList [formattedLeftItm; atom "."])
                            (self#longident_loc li)
                           in
                        SpecificInfixPrecedence
                          ({ reducePrecedence = prec; shiftPrecedence = prec
                           }, (LayoutNode layout))
                    | Pexp_construct (li,Some eo) when
                        not (is_simple_construct (view_expr x)) ->
                        (match view_expr x with
                         | `normal ->
                             let arityIsClear = isArityClear arityAttrs  in
                             FunctionApplication
                               [self#constructor_expression ~arityIsClear
                                  stdAttrs (self#longident_loc li) eo]
                         | _ -> assert false)
                    | Pexp_variant (l,Some eo) ->
                        ...
                        then
                          ...
                          FunctionApplication
                            [self#constructor_expression ~polyVariant:true
                               ~arityIsClear:true stdAttrs (atom ("`" ^ l))
                               eo]
                    ...
                    | Pexp_setfield (leftExpr,li,rightExpr) ->
                        ...
                        let leftItm =
                          self#unparseResolvedRule
                            (self#ensureExpression
                               ~reducesOnToken:(Token ".") leftExpr)
                           in
                        let leftLbl =
                          label (makeList [leftItm; atom "."])
                            (self#longident_loc li)
                           in
                        let expr =
                          label ~space:true
                            (makeList ~postSpace:true
                               [leftLbl; atom updateToken]) rightItm
                           in
                        SpecificInfixPrecedence
                          ({
                             reducePrecedence = (Token updateToken);
                        ...
                        (match detectTernary l with
                         ...
                         | Some (tt,ff) ->
                             ...
                             let trueFalseBranches =
                               makeList ~inline:(true, true) ~break:IfNeed
                                 ~sep:(Sep ":") ~postSpace:true
                                 ~preSpace:true [ifTrue; ifFalse]
                                in
                             let expr =
                               label ~space:true withQuestion
                                 trueFalseBranches
                                in
                             SpecificInfixPrecedence
                               ({
                    ...
                    | _ ->
                        (match self#expression_requiring_parens_in_infix x
                         with
                         | Some e -> e
                         | None  ->
                             raise
                               (Invalid_argument
                                  "No match for unparsing expression"))))
          method formatNonSequencyExpression e =
            match e.pexp_desc with
            | Pexp_record _|Pexp_tuple _|Pexp_object { pcstr_fields = [] }
              |Pexp_construct ({ txt = Lident "::" },Some _)|Pexp_construct
              ({ txt = Lident "[]" },_)|Pexp_extension ({ txt = "bs.obj" },_)
            ...
            let rec processArguments arguments processedAttrs children =
              ...
              | (Labelled
                 "children",{
                  ...
                  ->
                  processArguments tail processedAttrs
                    (self#formatChildren components [])
              | (Labelled "children",expr)::tail ->
                  let childLayout =
                    self#simplifyUnparseExpr ~wrap:("{", "}") expr  in
                  let dotdotdotChild =
                    makeList ~break:Layout.Never [atom "..."; childLayout]
                     in
                  processArguments tail processedAttrs
                    (Some [dotdotdotChild])
                    ...
                    | _ when isJSXComponent expression ->
                        label (atom (lbl ^ "="))
                          (makeList ~break:IfNeed ~wrap:("{", "}")
                             [self#simplifyUnparseExpr expression])
                    | Pexp_open (ovf,lid,e) when
                    ...
                    | Pexp_apply (eFun,_) ->
                        ...
                        let rhs =
                          ...
                          | _ ->
                           ...
                           in
                        label lhs rhs
                    | Pexp_record _|Pexp_construct _|Pexp_array _|Pexp_tuple
                      _|Pexp_match _|Pexp_extension _|Pexp_fun _
                      |Pexp_function _ ->
                        label (makeList [atom lbl; atom "="])
                          (self#simplifyUnparseExpr ~wrap:("{", "}")
                             expression)
                    | _ ->
                        makeList
              ...
              | _::tail -> processArguments tail processedAttrs children  in
            let (reversedAttributes,children) = processArguments args [] None
               in
            match children with
            | None  ->
                makeList ~break:IfNeed ~wrap:(("<" ^ componentName), "/>")
                  ~pad:(true, true) ~inline:(false, false) ~postSpace:true
            ...
            | Some renderedChildren ->
                let openTagAndAttrs =
                  ...
                  | revAttrHd::revAttrTl ->
                      let finalAttrList =
                        List.rev
                          ((makeList ~break:Layout.Never
                              [revAttrHd; atom ">"]) :: revAttrTl)
                         in
                      let renderedAttrList =
                        makeList ~inline:(true, true) ~break:IfNeed
                          ~pad:(false, false) ~preSpace:true finalAttrList
                         in
                      label ~space:true (atom ("<" ^ componentName))
                        renderedAttrList
                   in
                label openTagAndAttrs
                  (makeList
                     ~wrap:("",
                             ("</" ^
                                ((match closeComponentName with
                                  | None  -> componentName
                                  | Some close -> close) ^ ">")))
                     ~inline:(true, false) ~break:IfNeed ~pad:(true, true)
                     ~postSpace:true renderedChildren)[@@ocaml.doc
          ...
          method moduleExpressionToFormattedApplicationItems ?(prefix= "")  x
            ...
            | {
                ...
                let arg =
                  source_map ~loc:(m2.pmod_loc)
                    (self#simple_module_expr ~hug:true m2)
                   in
                label name arg
            | _ ->
                let rec extract_apps args =
                  ...
                  | { pmod_desc = Pmod_apply (me1,me2) } ->
                      let arg =
                        source_map ~loc:(me2.pmod_loc)
                          (self#simple_module_expr me2)
                         in
                      extract_apps (arg :: args) me1
                  | me ->
                      let head =
                        source_map ~loc:(me.pmod_loc) (self#module_expr me)
                         in
          ...
          method formatSimplePatternBinding labelOpener layoutPattern
            typeConstraint appTerms =
            let letPattern =
              label ~break:`Never ~space:true (atom labelOpener)
                layoutPattern
               in
            let upUntilEqual =
              match typeConstraint with
              | None  -> letPattern
              | Some tc -> formatTypeConstraint letPattern tc  in
            let includingEqual =
              makeList ~postSpace:true [upUntilEqual; atom "="]  in
            formatAttachmentApplication applicationFinalWrapping
              (Some (true, includingEqual)) appTerms
          method formatSimpleSignatureBinding labelOpener bindingPattern
            typeConstraint =
            let letPattern =
              label ~space:true (atom labelOpener) bindingPattern  in
            formatTypeConstraint letPattern typeConstraint
          method wrapCurriedFunctionBinding ?attachTo  ~arrow  ?(sweet=
            false)  ?(spaceBeforeArrow= true)  prefixText bindingLabel
            patternList returnedAppTerms =
            ...
            let everythingButReturnVal =
              match partitioning with
              | None  when sweet ->
                  makeList ~pad:(false, spaceBeforeArrow) ~wrap:("", arrow)
                    ~indent:(settings.space *
                               settings.indentWrappedPatternArgs)
                    ~postSpace:true ~inline:(true, true) ~break:IfNeed
                    allPatterns
              | None  ->
                  makeList ~pad:(true, spaceBeforeArrow)
                  ...
                  label ~space:true
                    (makeList ~pad:(true, spaceBeforeArrow)
                       ...
                       ~indent:(settings.space *
                                  settings.indentWrappedPatternArgs)
                       ~postSpace:true ~inline:(true, true) ~break:IfNeed
                       attachedList) wrappedListy
               in
            let everythingButAppTerms =
              match attachTo with
              | None  -> everythingButReturnVal
              | Some toThis ->
                  label ~space:true toThis everythingButReturnVal
               in
            formatAttachmentApplication applicationFinalWrapping
              (Some (true, everythingButAppTerms)) returnedAppTerms
          ...
          method curriedPatternsAndReturnVal x =
            let uncurried =
              try Hashtbl.find uncurriedTable x.pexp_loc
              with | Not_found  -> false  in
            ...
            let prepare_arg =
              ...
              | `Type nt -> atom ("type " ^ nt)  in
            let single_argument_no_parens p =
              ((is_unit_pattern p) ||
                 ((is_ident_pattern p) || (is_any_pattern p)))
                && (not uncurried)
               in
            match extract_args x with
            | ([],ret) -> ([], ret)
            | ((`Value (Nolabel ,None ,p))::[],ret) when
                (is_unit_pattern p) && uncurried -> ([atom "(.)"], ret)
            | ((`Value (Nolabel ,None ,p) as arg)::[],ret) when
                single_argument_no_parens p -> ([prepare_arg arg], ret)
            | (args,ret) ->
                if uncurried
                then
                  ([makeTup ~uncurried:true (List.map prepare_arg args)],
                    ret)
                else ([makeTup (List.map prepare_arg args)], ret)
          method curriedFunctorPatternsAndReturnStruct =
            ...
            | { pmod_desc = Pmod_functor (s,mt,me2) } ->
                let firstOne =
                  ...
                  | None  -> atom "()"
                  | Some mt' ->
                      self#module_type (makeList [atom s.txt; atom ":"]) mt'
                   in
                let (functorArgsRecurse,returnStructure) =
                  self#curriedFunctorPatternsAndReturnStruct me2  in
                ((firstOne :: functorArgsRecurse), returnStructure)
            | me -> ([], me)
          method isRenderableAsPolymorphicAbstractTypes typeVars polyType
            leadingAbstractVars nonVarifiedType =
            (same_ast_modulo_varification_and_extensions polyType
               nonVarifiedType)
              && (for_all2' string_equal typeVars leadingAbstractVars)
          method locallyAbstractPolymorphicFunctionBinding prefixText
            layoutPattern funWithNewTypes absVars bodyType =
            ...
            let polyType =
              label ~space:true
                (makeList
                   [makeList ~sep:(Sep " ") ((atom "type") ::
                      locallyAbstractTypes);
                   atom "."]) typeLayout
               in
            self#formatSimplePatternBinding prefixText layoutPattern
              (Some polyType) appTerms
          method wrappedBinding prefixText ~arrow  pattern patternAux expr =
            ...
            let patternList =
              ...
              | [] -> pattern
              | _::_ ->
                  makeList ~postSpace:true ~inline:(true, true) ~break:IfNeed
                    (pattern :: patternAux)
               in
            ...
            | ([],Pexp_constraint (e,ct)) ->
                ...
                self#formatSimplePatternBinding prefixText patternList
                  (Some typeLayout) appTerms
            | ([],_) ->
            ...
            | (_::_,_) ->
                let (argsWithConstraint,actualReturn) =
                  self#normalizeFunctionArgsConstraint argsList return  in
                let fauxArgs = List.concat [patternAux; argsWithConstraint]
                   in
                let returnedAppTerms =
                  self#unparseExprApplicationItems actualReturn  in
                let lbl =
                  makeList ~sep:(Sep " ") ~break:Layout.Never
                    [pattern; atom "="]
                   in
                self#wrapCurriedFunctionBinding prefixText ~arrow lbl
                  fauxArgs returnedAppTerms[@@ocaml.doc
          ...
          method wrappedClassBinding prefixText pattern patternAux expr =
            let (args,return) =
              self#curriedConstructorPatternsAndReturnVal expr  in
            let patternList =
              match patternAux with
              | [] -> pattern
              | _::_ ->
            ...
            | (None ,_) ->
                self#formatSimplePatternBinding prefixText patternList None
                  ((self#classExpressionToFormattedApplicationItems expr),
                    None)
            | (Some args,_) ->
                let (argsWithConstraint,actualReturn) =
                ...
                let fauxArgs = List.concat [patternAux; argsWithConstraint]
                   in
                self#wrapCurriedFunctionBinding prefixText ~arrow:"=" pattern
                  fauxArgs
                  ((self#classExpressionToFormattedApplicationItems
                      actualReturn), None)
          method attachDocAttrsToLayout
            ~stdAttrs:(stdAttrs : Ast_404.Parsetree.attributes) 
            ~docAttrs:(docAttrs : Ast_404.Parsetree.attributes)  ~loc 
            ~layout  () =
            let loc =
              match stdAttrs with | (astLoc,_)::_ -> astLoc.loc | [] -> loc
               in
            let rec aux prevLoc layout =
              function
              | (((x,_) as attr) : Ast_404.Parsetree.attribute)::xs ->
                  let newLayout =
                    ...
                    let layout =
                      if
                        Range.containsWhitespace ~range
                          ~comments:(self#comments) ()
                      then
                        let region =
                          WhitespaceRegion.make ~range ~newlines:1 ()  in
            ...
            let body =
              ...
              | Ppat_constraint (p,ty) ->
                  ...
                  let leadingAbsTypesAndExpr =
                    self#leadingCurriedAbstractTypes x.pvb_expr  in
                  (match ((p.ppat_desc), (ty.ptyp_desc),
                           leadingAbsTypesAndExpr)
                   with
                   | (Ppat_var s,Ptyp_poly
                      (typeVars,varifiedPolyType),((_::_ as absVars),Pexp_constraint
                                                   (funWithNewTypes,nonVarifiedExprType)))
                       when
                       self#isRenderableAsPolymorphicAbstractTypes typeVars
                         varifiedPolyType absVars nonVarifiedExprType
                       ->
                       self#locallyAbstractPolymorphicFunctionBinding
                         prefixText layoutPattern funWithNewTypes absVars
                         nonVarifiedExprType
                   | _ ->
                       let typeLayout =
                         source_map ~loc:(ty.ptyp_loc) (self#core_type ty)
                          in
                       let appTerms =
                         self#unparseExprApplicationItems x.pvb_expr  in
                       self#formatSimplePatternBinding prefixText
                         layoutPattern (Some typeLayout) appTerms)
              | _ ->
                  let layoutPattern =
                    source_map ~loc:((x.pvb_pat).ppat_loc)
                      (self#pattern x.pvb_pat)
                     in
                  let appTerms = self#unparseExprApplicationItems x.pvb_expr
                     in
                  self#formatSimplePatternBinding prefixText layoutPattern
                    None appTerms
               in
            let { stdAttrs; docAttrs } =
              partitionAttributes ~partDoc:true x.pvb_attributes  in
            ...
            | Pexp_constraint (e,ct) ->
                let typeLayout =
                  source_map ~loc:(ct.ptyp_loc)
                   ...
                   in
                ([makeList ~break:IfNeed ~inline:(true, true)
          ...
          method bindingsLocationRange ?extension  l =
            ...
            let fstLoc =
              ...
              | Some ({ pexp_loc = { loc_ghost = false  } } as ext) ->
                  ext.pexp_loc
              | _ -> (List.nth l 0).pvb_loc  in
          ...
          method bindings ?extension  (rf,l) =
            ...
            let label =
              match rf with
              | Nonrecursive  -> label
              | Recursive  -> label ^ " rec"  in
            let first = self#binding label first  in
            match rest with
            | [] -> first
            | _ ->
                makeList ~postSpace:true ~break:Always ~indent:0
                  ~inline:(true, true) (first ::
                  (List.map (self#binding "and") rest))
          method letList expr =
            let rec processLetList acc expr =
              ...
              | (attrs,Pexp_open (ovf,lid,e)) when
                  not
                    (self#isSeriesOfOpensFollowedByNonSequencyExpression
                       { expr with pexp_attributes = [] })
                  ->
                  let overrideStr =
                    match ovf with | Override  -> "!" | Fresh  -> ""  in
                  let openLayout =
                    label ~space:true (atom ("open" ^ overrideStr))
                      (self#longident_loc lid)
                     in
                  let attrsOnOpen =
                    makeList ~inline:(true, true) ~postSpace:true
                      ~break:Always ((self#attributes attrs) @ [openLayout])
                     in
              ...
              | ([],Pexp_letmodule (s,me,e)) ->
                  ...
                  let layout = source_map ~loc:letModuleLoc letModuleLayout
                     in
                  let (_,return) =
                    self#curriedFunctorPatternsAndReturnStruct moduleExpr  in
                  let loc =
                    { letModuleLoc with loc_end = ((return.pmod_loc).loc_end)
                    }  in
                  processLetList ((loc, layout) :: acc) e
              | ([],Pexp_letexception (extensionConstructor,expr)) ->
                  let exc = self#exception_declaration extensionConstructor
                     in
                  let layout =
                    source_map ~loc:(extensionConstructor.pext_loc) exc  in
                  processLetList (((extensionConstructor.pext_loc), layout)
                    :: acc) expr
              | ([],Pexp_sequence
                 (({ pexp_desc = Pexp_sequence _ } as e1),e2))
                |([],Pexp_sequence (({ pexp_desc = Pexp_let _ } as e1),e2))
                |([],Pexp_sequence (({ pexp_desc = Pexp_open _ } as e1),e2))
                |([],Pexp_sequence
                  (({ pexp_desc = Pexp_letmodule _ } as e1),e2))
                |([],Pexp_sequence (e1,e2)) ->
                  let e1Layout =
              ...
              | _ ->
                  (match expression_not_immediate_extension_sugar expr with
                   | Some
                       (extension,{ pexp_attributes = [];
                                    pexp_desc = Pexp_let (rf,l,e) })
                       ->
                       let bindingsLayout = self#bindings ~extension (rf, l)
                          in
                       let bindingsLoc =
                         self#bindingsLocationRange ~extension:expr l  in
                       let layout =
                         source_map ~loc:bindingsLoc bindingsLayout  in
                       processLetList
                         (((extractLocationFromValBindList expr l), layout)
                         :: acc) e
                   | Some (extension,e) ->
                       let layout =
                         self#attach_std_item_attrs ~extension []
                   ...
                   | None  ->
                       let layout =
                         source_map ~loc:(expr.pexp_loc)
                           (self#unparseExpr expr)
                          in
                       ((expr.pexp_loc), layout) :: acc)
               in
            let es = processLetList [] expr  in
            groupAndPrint ~xf:(fun (_,layout)  -> layout)
              ~getLoc:(fun (loc,_)  -> loc) ~comments:(self#comments)
              (List.rev es)
          method constructor_expression ?(polyVariant= false)  ~arityIsClear 
            stdAttrs ctor eo =
            let (implicit_arity,arguments) =
              ...
              | Pexp_tuple l when polyVariant == true ->
                  (false,
                    (self#unparseSequence ~wrap:("(", ")") ~construct:`Tuple
                       l))
              | Pexp_tuple l ->
                  (match l with
              ...
              | _ -> (false, (makeTup [self#unparseConstraintExpr eo]))  in
            ...
            let attrs =
              ...
              then
                ({ txt = "implicit_arity"; loc = (eo.pexp_loc) }, (PStr []))
                :: stdAttrs
              else stdAttrs  in
            match attrs with
            | [] -> construction
            | _::_ -> formatAttributed construction (self#attributes attrs)
          method constructor_pattern ?(polyVariant= false)  ~arityIsClear 
            ctor po =
            let (implicit_arity,arguments) =
            ...
            let (space,arguments) =
              match arguments with
              | x::[] when is_direct_pattern x ->
                  (true, (self#simple_pattern x))
              | xs when isSingleArgParenPattern xs ->
                  (false, (self#singleArgParenPattern xs))
              | ({ ppat_desc = Ppat_constant (Pconst_string (s,None )) } as x)::[]
                |({ ppat_desc = Ppat_construct ({ txt = Lident s },None ) }
                    as x)::[]|({ ppat_desc = Ppat_var { txt = s } } as x)::[]
            ...
            then
              formatAttributed construction
                (self#attributes
                   [({ txt = "implicit_arity"; loc = (po.ppat_loc) },
                      (PStr []))])
            else construction
          method singleArgParenPattern =
            ...
            | ({ ppat_desc = Ppat_construct ({ txt = Lident "::" },po);
                 ppat_loc } as listPattern)::[]
                ->
                source_map ~loc:ppat_loc
                  (self#patternList ~wrap:("(", ")") listPattern)
            | _ -> assert false
          method patternArray ?(wrap= ("", ""))  l =
            let (left,right) = wrap  in
            let wrap = ((left ^ "[|"), ("|]" ^ right))  in
            makeList ~wrap ~break:IfNeed ~postSpace:true ~sep:commaTrail
              (List.map self#pattern l)
          method patternTuple ?(wrap= ("", ""))  l =
            let (left,right) = wrap  in
            let wrap = ((left ^ "("), (")" ^ right))  in
            makeList ~wrap ~sep:commaTrail ~postSpace:true ~break:IfNeed
              (List.map self#constrained_pattern l)
          method patternRecord ?(wrap= ("", ""))  l closed =
            let longident_x_pattern (li,p) =
              match (li, (p.ppat_desc)) with
              | ({ txt = ident },Ppat_var { txt }) when
                  (Longident.last ident) = txt -> self#longident_loc li
              | ({ txt = ident },Ppat_alias
          ...
          method expression_requiring_parens_in_infix x =
            ...
            (let originalExpr = x  in
             ...
             match x.pexp_desc with
             | Pexp_function _ when pipe || semi -> None
             | Pexp_function l ->
             ...
             | _ ->
                 let itm =
                   ...
                   | Pexp_fun _|Pexp_newtype _ ->
                       ...
                       (match args with
                        ...
                        | firstArg::tl ->
                            let retValUnparsed =
                              self#unparseExprApplicationItems ret  in
                            Some
                   ...
                   | Pexp_try (e,l) ->
                       ...
                       let switchWith =
                         label ~space:true
                          ...
                          in
                       Some
                         (label ~space:true switchWith
                            (source_map ~loc:estimatedBracePoint
                               (makeList ~indent:(settings.trySwitchIndent)
                                  ~wrap:("{", "}") ~break:Always_rec
                                  ~postSpace:true cases)))
                   | Pexp_setinstvar (s,e) ->
                       raise
                         (Invalid_argument
                         ...
                         | (hd::tl,_) ->
                             ...
                             let nextSoFar =
                               label ~space:true soFarWithElseIfAppended
                                 (source_map ~loc:(e2.pexp_loc)
                                    (makeLetSequence (self#letList e2)))
                                in
                             sequence nextSoFar tl
                          in
                       let init =
                         let if_ = atom (add_extension_sugar "if" extension)
                            in
                         let cond = self#parenthesized_expr e1  in
                         label ~space:true
                           (source_map ~loc:(e1.pexp_loc)
                              (label ~space:true if_ cond))
                           (source_map ~loc:(e2.pexp_loc)
                              (makeLetSequence (self#letList e2)))
                          in
                   ...
                   | Pexp_while (e1,e2) ->
                       let lbl =
                         let while_ =
                           atom (add_extension_sugar "while" extension)  in
                         let cond = self#parenthesized_expr e1  in
                         label ~space:true (label ~space:true while_ cond)
                           (source_map ~loc:(e2.pexp_loc)
                              (makeLetSequence (self#letList e2)))
                          in
                       Some lbl
                   | Pexp_for (s,e1,e2,df,e3) ->
                       ...
                       let upToBody =
                         makeList ~inline:(true, true) ~postSpace:true
                           [atom (add_extension_sugar "for" extension);
                           dockedToFor]
                          in
                       Some
                         (label ~space:true upToBody
                            (source_map ~loc:(e3.pexp_loc)
                               (makeLetSequence (self#letList e3))))
                   | Pexp_new li ->
                       Some
                         (label ~space:true (atom "new")
                 ...
                 (match itm with
                  ...
                  | Some i ->
                      Some
                        (PotentiallyLowPrecedence
                           (source_map ~loc:(x.pexp_loc) i))))
          ...
          method simple_enough_to_be_lhs_dot_send x =
            ...
            | Pexp_apply (eFun,_) ->
                (match printedStringAndFixityExpr eFun with
                 ...
                 | Normal  ->
                     if x.pexp_attributes = []
                     then self#unparseExpr x
                     else self#simplifyUnparseExpr x)
            | _ -> self#simplifyUnparseExpr x
          method unparseRecord ?wrap:((lwrap,rwrap)= ("", "")) 
            ?(withStringKeys= false)  ?(allowPunning= true)  ?(forceBreak=
            false)  l eo =
            ...
            let maybeQuoteFirstElem fst rest =
              ...
              else (self#longident_loc fst) :: rest  in
            let makeRow (li,e) shouldPun =
              ...
              let theRow =
                ...
                | (Pexp_ident { txt = Lident value },true ,true ) when
                    (Longident.last li.txt) = value ->
                    makeList (maybeQuoteFirstElem li [])
                | (Pexp_record (recordRows,optionalGadt),_,_) ->
                    (forceBreak := true;
                     (let keyWithColon =
                        makeList (maybeQuoteFirstElem li [atom ":"])  in
                      let value =
                        self#unparseRecord ~forceBreak:true recordRows
                          optionalGadt
                         in
                ...
                | (Pexp_extension (s,p),_,_) when s.txt = "bs.obj" ->
                    (forceBreak := true;
                     (let keyWithColon =
                        makeList (maybeQuoteFirstElem li [atom ":"])  in
                      let value =
                        self#formatBsObjExtensionSugar ~forceBreak:true p  in
                      label ~space:true keyWithColon value))
                ...
                | _ ->
                    let (argsList,return) =
                      self#curriedPatternsAndReturnVal e  in
                    (match argsList with
                     | [] ->
                         ...
                         let upToColon =
                           makeList (maybeQuoteFirstElem li [atom ":"])  in
                         formatAttachmentApplication applicationFinalWrapping
                           (Some (true, upToColon)) appTerms
                     | firstArg::tl ->
                         let upToColon =
                           makeList (maybeQuoteFirstElem li [atom ":"])  in
                         let returnedAppTerms =
                           self#unparseExprApplicationItems return  in
                         self#wrapCurriedFunctionBinding ~sweet:true
                           ~attachTo:upToColon funToken ~arrow:"=>" firstArg
                           tl returnedAppTerms)
                 in
              source_map ~loc:totalRowLoc theRow  in
            let rec getRows l =
              match l with
              | [] -> []
              | hd::[] -> [makeRow hd true]
              | hd::hd2::tl -> (makeRow hd true) :: (getRows (hd2 :: tl))  in
            let allRows =
              match eo with
              | None  ->
                  (match l with
              ...
              | Some withRecord ->
                  let firstRow =
                    let appTerms =
                      self#unparseExprApplicationItems withRecord  in
                    formatAttachmentApplication applicationFinalWrapping
                      (Some (false, (atom "..."))) appTerms
                     in
                  (source_map ~loc:(withRecord.pexp_loc) firstRow) ::
                    (getRows l)
               in
            makeList ~wrap:((lwrap ^ "{"), ("}" ^ rwrap))
              ~break:(if !forceBreak then Layout.Always else Layout.IfNeed)
              ~sep:commaTrail ~postSpace:true allRows
          method isSeriesOfOpensFollowedByNonSequencyExpression expr =
            ...
            | _ -> true
          method unparseObject ?wrap:((lwrap,rwrap)= ("", "")) 
            ?(withStringKeys= false)  l o =
            let core_field_type (s,attrs,ct) =
              let l = extractStdAttrs attrs  in
              let row =
                let rowKey =
                  if withStringKeys
                  then makeList ~wrap:("\"", "\"") [atom s]
                  else atom s  in
                label ~space:true
            ...
            let openness =
              match o with | Closed  -> atom "." | Open  -> atom ".."  in
            let rows_layout =
              makeList ~inline:(true, true) ~postSpace:true ~sep:commaTrail
                rows
                ~break:(if (List.length rows) >= 2
                        then Layout.Always_rec
                        else Layout.IfNeed)
               in
            makeList ~break:Layout.IfNeed ~preSpace:(rows != [])
              ~wrap:((lwrap ^ "{"), ("}" ^ rwrap)) [openness; rows_layout]
          method unparseSequence ?(wrap= ("", ""))  ~construct  l =
            ...
            | `ES6List ->
                let (seq,ext) =
                  ...
                  | [] -> assert false  in
                makeES6List ~wrap (List.map self#unparseExpr seq)
            ...
            | _ ->
                ...
                let (xf,(leftDelim,rightDelim)) =
                  ...
                  | `ES6List -> assert false  in
                let wrap = ((left ^ leftDelim), (rightDelim ^ right))  in
                makeList ~wrap ~sep:commaTrail ~break:IfNeed ~postSpace:true
                  (List.map xf l)
          method formatBsObjExtensionSugar ?(wrap= ("", ""))  ?(forceBreak=
            false)  payload =
            match payload with
            | PStr (itm::[]) ->
                (match itm with
                 | {
                     pstr_desc = Pstr_eval
                       ({ pexp_desc = Pexp_record (l,eo) },[])
                     } ->
                     self#unparseRecord ~forceBreak ~wrap
                       ~withStringKeys:true ~allowPunning:false l eo
                 | {
                     pstr_desc = Pstr_eval
                       ({
                 ...
                 | _ ->
                     raise
                       (Invalid_argument
                          "bs.obj only accepts a record. You've passed something else"))
            | _ -> assert false
            ...
            then None
              ...
              (let item =
                 ...
                 | Pexp_function l when pipe || semi ->
                     Some
                       (formatPrecedence ~loc:(x.pexp_loc)
                 ...
                 | Pexp_override l ->
                     let string_x_expression (s,e) =
                       label ~space:true (atom (s.txt ^ ":"))
                         (self#unparseExpr e)
                        in
                     Some
                       (makeList ~postSpace:true ~wrap:("{<", ">}")
                          ~sep:(Sep ",") (List.map string_x_expression l))
                         ...
                         | `list xs ->
                             ...
                             then
                               let actualChildren =
                                 ...
                                 | None  -> []
                                 | Some ch -> ch  in
                               makeList ~break:IfNeed ~inline:(false, false)
                                 ~postSpace:true ~wrap:("<>", "</>")
                                 ~pad:(true, true) actualChildren
                             else self#unparseSequence ~construct:`List xs
                         | `cons xs ->
                 ...
                 | Pexp_constant c ->
                     let (raw_literal,_) =
                       extract_raw_literal x.pexp_attributes  in
                     Some
                       (ensureSingleTokenSticksToLabel
                          (self#constant ?raw_literal c))
                 | Pexp_pack me ->
                 ...
                 | Pexp_constraint (e,ct) ->
                     Some
                       (makeList ~break:IfNeed ~wrap:("(", ")")
                          [formatTypeConstraint (self#unparseExpr e)
                             (self#core_type ct)])
                 | Pexp_coerce (e,cto1,ct) ->
                     ...
                     Some
                       (makeList ~break:IfNeed ~wrap:("(", ")")
                          [formatCoerce (self#unparseExpr e) optFormattedType
                             (self#core_type ct)])
                 | Pexp_variant (l,None ) ->
                 ...
                 | Pexp_open (ovf,lid,e) ->
                     ...
                     then
                       Some
                         (label (label (self#longident_loc lid) (atom "."))
                            (self#formatNonSequencyExpression e))
          ...
          method formatChildren children processedRev =
            ...
            | ({ pexp_desc = Pexp_constant constant } as x)::remaining ->
                let (raw_literal,_) = extract_raw_literal x.pexp_attributes
                   in
                self#formatChildren remaining
                  ((self#constant ?raw_literal constant) :: processedRev)
            | ({
                 pexp_desc = Pexp_construct
                   ({ txt = Lident "::" },Some
                    { pexp_desc = Pexp_tuple children })
                 } as x)::remaining
                ->
                (match x.pexp_attributes with
                 | ({ txt = "JSX"; loc },PStr [])::_ ->
                     (match self#simplest_expression x with
                      | Some r ->
                          self#formatChildren remaining (r :: processedRev)
                      | None  ->
                          self#formatChildren (remaining @ children)
                            processedRev)
                 | _ ->
            ...
            | ({ pexp_desc = Pexp_apply (expr,l); pexp_attributes } as e)::remaining
                ->
                let child =
                  if Reason_heuristics.isFastPipeWithApplicationJSXChild e
                  then
                    formatPrecedence ~wrap:("{", "}") (self#formatFastPipe e)
                  else self#simplifyUnparseExpr ~wrap:("{", "}") e  in
                self#formatChildren remaining (child :: processedRev)
            | { pexp_desc = Pexp_ident li }::remaining ->
                self#formatChildren remaining ((self#longident_loc li) ::
                  processedRev)
            | { pexp_desc = Pexp_construct ({ txt = Lident "[]" },None ) }::remaining
                -> self#formatChildren remaining processedRev
            | head::remaining ->
                self#formatChildren remaining
                  ((self#simplifyUnparseExpr ~wrap:("{", "}") head) ::
                  processedRev)
            | [] ->
          ...
          method payload ppxToken ppxId e =
            ...
            | PStr (_::_ as items) ->
                ...
                makeList ~wrap ~break ~pad ~postSpace ~sep:(Layout.Sep ";")
                  rows
            | PTyp x ->
          ...
          method attribute =
            ...
            | ({ Location.txt = ("ocaml.doc"|"ocaml.text") },PStr
               ({
                  pstr_desc = Pstr_eval
                    ({
                       pexp_desc = Pexp_constant (Pconst_string (text,None ));_},_);
                  pstr_loc;_}::[]))
                ->
                let text =
                  if text = "" then "/**/" else "/**" ^ (text ^ "*/")  in
                makeList ~inline:(true, true) ~postSpace:true ~preSpace:true
                  ~indent:0 ~break:IfNeed [atom ~loc:pstr_loc text]
          ...
          method attach_std_item_attrs ?(allowUncurry= true)  ?extension  l
            toThis =
            ...
            | (_,_) ->
                let extension =
                  match extension with
                  | None  -> []
                  | Some id -> [atom ("%" ^ id.txt)]  in
                makeList ~postSpace:true ~indent:0 ~break:Layout.Always_rec
                  ~inline:(true, true)
          ...
          method exception_declaration ed =
            ...
            let exn_arg =
              ...
              | Pext_rebind id ->
               ...
               in
            let { stdAttrs; docAttrs } =
              partitionAttributes ~partDoc:true ed.pext_attributes  in
            let layout =
              self#attach_std_item_attrs stdAttrs
          ...
          method class_sig_field x =
            ...
            | Pctf_inherit ct ->
                label ~space:true (atom "inherit")
                  (self#class_constructor_type ct)
            | Pctf_val (s,mf,vf,ct) ->
                let valueFlags = self#value_type_flags_for (s ^ ":") (vf, mf)
                   in
                label ~space:true
                  (label ~space:true (atom "val")
                     (makeList ~postSpace:true ~inline:(false, true)
                        ~break:IfNeed valueFlags)) (self#core_type ct)
            | Pctf_method (s,pf,vf,ct) ->
                let methodFlags = self#method_sig_flags_for (s ^ ":") vf  in
                let pubOrPrivate =
                  match pf with | Private  -> "pri" | Public  -> "pub"  in
                let m =
                  label ~space:true
                    (label ~space:true (atom pubOrPrivate)
                       (makeList ~postSpace:true ~inline:(false, true)
                          ~break:IfNeed methodFlags)) (self#core_type ct)
                   in
                self#attach_std_item_attrs x.pctf_attributes m
            | Pctf_constraint (ct1,ct2) ->
                label ~space:true (atom "constraint")
                  (label ~space:true
          ...
          method primitive_declaration vd =
            let lblBefore =
              label ~space:true
                (makeList
                   [makeList ~postSpace:true
                      [atom "external"; protectIdentifier (vd.pval_name).txt];
                   atom ":"]) (self#core_type vd.pval_type)
               in
            let frstHalf = makeList ~postSpace:true [lblBefore; atom "="]  in
            let sndHalf =
              makeSpacedBreakableInlineList
                (List.map self#constant_string vd.pval_prim)
               in
          ...
          method class_instance_type x =
            ...
            | Pcty_signature cs ->
                ...
                let allItems =
                  ...
                  | _ -> (label ~space:true (atom "as") (self#core_type ct))
                      :: instTypeFields
                   in
                self#attach_std_item_attrs ~allowUncurry:false
                  x.pcty_attributes
                  (makeList ~wrap:("{", "}") ~postSpace:true
            ...
            | Pcty_arrow _ ->
                failwith
                  "class_instance_type should not be printed with Pcty_arrow"
          method class_declaration_list l =
            let class_declaration ?(class_keyword= false) 
              ({ pci_params = ls; pci_name = { txt }; pci_virt;
                 pci_expr = { pcl_desc }; pci_loc } as x)
              =
              let (firstToken,pattern,patternAux) =
                self#class_opening class_keyword txt pci_virt ls  in
              let classBinding =
                self#wrappedClassBinding firstToken pattern patternAux
                  x.pci_expr
                 in
            ...
            | [] ->
                raise
                  (NotPossible
                     "Class definitions will have at least one item.")
            | x::rest ->
          ...
          method class_type_declaration_list l =
            let class_type_declaration kwd
              ...
              let upToName =
                ...
                then label ~space:true (atom opener) (atom pci_name.txt)
                 ...
                 in
              let includingEqual =
                makeList ~postSpace:true [upToName; atom "="]  in
              let { stdAttrs; docAttrs } =
                partitionAttributes ~partDoc:true pci_attributes  in
              let layout =
                (self#attach_std_item_attrs stdAttrs) @@
                  (label ~space:true includingEqual
                     (self#class_instance_type x.pci_expr))
                 in
              self#attachDocAttrsToLayout ~stdAttrs ~docAttrs
                ~loc:(pci_name.loc) ~layout ()
               in
            match l with
            | [] ->
                failwith
                  "Should not call class_type_declaration with no classes"
            | x::[] -> class_type_declaration "class type" x
            | x::xs ->
                makeList ~break:Always_rec ~indent:0 ~inline:(true, true)
                  ((class_type_declaration "class type" x) ::
                  (List.map (class_type_declaration "and") xs))
          method class_constructor_type x =
            match x.pcty_desc with
            | Pcty_arrow (l,co,cl) ->
                let rec allArrowSegments acc =
                  function
                  | { pcty_desc = Pcty_arrow (l,ct1,ct2) } ->
                      allArrowSegments
                        ((self#type_with_label (l, ct1, false)) :: acc) ct2
                  | xx -> ((List.rev acc), (self#class_constructor_type xx))
                   in
                let (params,return) = allArrowSegments [] x  in
                let normalized =
                  makeList ~break:IfNeed ~sep:(Sep "=>") ~preSpace:true
                    ~postSpace:true ~inline:(true, true)
          ...
          method class_field x =
            let itm =
              ...
              | Pcf_inherit (ovf,ce,so) ->
                  ...
                  label ~space:true (atom inheritText)
                    (match so with
                     | None  -> inheritExp
                     | Some s ->
                         label ~space:true inheritExp (atom ("as " ^ s)))
              | Pcf_val (s,mf,Cfk_concrete (ovf,e)) ->
                  let opening =
                    match mf with
                    | Mutable  ->
                        let mutableName = [atom "mutable"; atom s.txt]  in
                        label ~space:true (atom ("val" ^ (override ovf)))
                          (makeList ~postSpace:true ~inline:(false, true)
                             ~break:IfNeed mutableName)
                    | Immutable  ->
                        label ~space:true (atom ("val" ^ (override ovf)))
                          (atom s.txt)
                     in
                  ...
                  let opening =
                    ...
                    | Mutable  ->
                        let mutableVirtualName =
                          [atom "mutable"; atom "virtual"; atom s.txt]  in
                        let openingTokens =
                          makeList ~postSpace:true ~inline:(false, true)
                            ~break:IfNeed mutableVirtualName
                           in
              ...
              | Pcf_method (s,pf,Cfk_virtual ct) ->
                  let opening =
                    match pf with
                    | Private  ->
                        let privateVirtualName = [atom "virtual"; atom s.txt]
                           in
                        let openingTokens =
                          makeList ~postSpace:true ~inline:(false, true)
                            ~break:IfNeed privateVirtualName
                           in
                  ...
                  (match e.pexp_desc with
                   | Pexp_poly
                       ({
                        ...
                        { ptyp_desc = Ptyp_poly (typeVars,varifiedPolyType) })
                       when
                       let (leadingAbstractVars,nonVarified) =
                         self#leadingCurriedAbstractTypes
                           methodFunWithNewtypes
                          in
                       self#isRenderableAsPolymorphicAbstractTypes typeVars
                         varifiedPolyType leadingAbstractVars
                         nonVarifiedExprType
                       ->
                       let (leadingAbstractVars,nonVarified) =
                         self#leadingCurriedAbstractTypes
                          ...
                          in
                       self#locallyAbstractPolymorphicFunctionBinding
                         methodText (atom s.txt) methodFunWithNewtypes
                         leadingAbstractVars nonVarifiedExprType
                   | Pexp_poly (e,Some ct) ->
                       self#formatSimplePatternBinding methodText
                         (atom s.txt)
                         (Some
                            (source_map ~loc:(ct.ptyp_loc)
                               (self#core_type ct)))
                   ...
                   | Pexp_poly (e,None ) ->
                       self#wrappedBinding methodText ~arrow:"=>"
                         (atom s.txt) [] e
                   | _ ->
                       failwith
                         "Concrete methods should only ever have Pexp_poly.")
              | Pcf_constraint (ct1,ct2) ->
                  label ~space:true (atom "constraint")
                    (makeList ~postSpace:true ~inline:(true, false)
                       [makeList ~postSpace:true
                          [self#core_type ct1; atom "="];
                       self#core_type ct2])
              | Pcf_initializer e ->
                  label ~space:true (atom "initializer")
                    (self#simplifyUnparseExpr e)
              | Pcf_attribute a -> self#floating_attribute a
              | Pcf_extension e -> self#item_extension e  in
          ...
          method class_self_pattern_and_structure
            ...
            | _ ->
                let field = label ~space:true (atom "as") (self#pattern p)
                   in
            ...
            then
              ...
              (let itm =
                 ...
                 | Pcl_constraint (ce,ct) ->
                     formatTypeConstraint (self#class_expr ce)
                       (self#class_constructor_type ct)
                 | Pcl_let _|Pcl_structure _ ->
                     let rows = self#classExprLetsAndRest x  in
                     makeList ~wrap:("{", "}") ~inline:(true, false)
                       ~postSpace:true ~break:Always_rec
                       (List.map semiTerminated rows)
                 | Pcl_extension e -> self#extension e
                 | _ -> formatPrecedence (self#class_expr x)  in
          ...
          method class_expr x =
            ...
            then
              ...
              (match x.pcl_desc with
               | Pcl_fun _ ->
                   (match self#curriedConstructorPatternsAndReturnVal x with
                    ...
                    | (Some args,e) ->
                        label ~space:true
                          (makeList ~postSpace:true
                             [label ~space:true (atom funToken) args;
                             atom "=>"]) (self#class_expr e))
               | Pcl_apply (ce,l) ->
                   formatAttachmentApplication applicationFinalWrapping None
                     ((self#classExpressionToFormattedApplicationItems x),
                       None)
               | Pcl_constr (li,[]) ->
               ...
               | Pcl_constraint _|Pcl_extension _|Pcl_let _|Pcl_structure _
                   -> self#simple_class_expr x)
          method classStructure ?(forceBreak= false)  ?(wrap= ("", ""))  cs =
            let (left,right) = wrap  in
            makeList ~sep:(Layout.Sep ";")
              ~wrap:((left ^ "{"), ("}" ^ right))
              ~break:(if forceBreak then Layout.Always else Layout.IfNeed)
              ~postSpace:true ~inline:(true, false)
          ...
          method signature signatureItems =
            ...
            | first::_ as signatureItems ->
                ...
                let items =
                  groupAndPrint ~xf:(self#signature_item)
                    ~getLoc:(fun x  -> x.psig_loc) ~comments:(self#comments)
                    signatureItems
                   in
                source_map ~loc:{ loc_start; loc_end; loc_ghost = false }
                  (makeList ~postSpace:true ~break:Layout.Always_rec
                     ~indent:0 ~inline:(true, false)
                     ~sep:(SepFinal (";", ";")) items)
          method signature_item x =
            (let item =
               (match x.psig_desc with
                ...
                | Psig_value vd ->
                    ...
                    then self#primitive_declaration vd
                      ...
                      (let intro = atom "let"  in
                       let { stdAttrs; docAttrs } =
                         partitionAttributes ~partDoc:true vd.pval_attributes
                          in
                       let layout =
                         self#attach_std_item_attrs stdAttrs
                    ...
                    let class_description ?(class_keyword= false) 
                      ...
                      let (firstToken,pattern,patternAux) =
                        self#class_opening class_keyword txt x.pci_virt ls
                         in
                      let withColon =
                        self#wrapCurriedFunctionBinding ~arrow:":"
                          ~spaceBeforeArrow:false firstToken pattern
                          patternAux
                          ([self#class_constructor_type x.pci_expr], None)
                         in
                      let { stdAttrs; docAttrs } =
                        partitionAttributes ~partDoc:true x.pci_attributes
                         in
                      let layout =
                        self#attach_std_item_attrs stdAttrs withColon  in
                      source_map ~loc:pci_loc
                        (self#attachDocAttrsToLayout ~stdAttrs ~docAttrs
                           ~loc:((x.pci_name).loc) ~layout ())
                       in
                    makeNonIndentedBreakingList
                      (match l with
                       | [] ->
                       ...
                       | x::xs -> (class_description ~class_keyword:true x)
                           :: (List.map class_description xs))
                | Psig_module
                    { pmd_name; pmd_type = { pmty_desc = Pmty_alias alias };
                      pmd_attributes; pmd_loc }
                    ->
                    let { stdAttrs; docAttrs } =
                      partitionAttributes ~partDoc:true pmd_attributes  in
                    let layout =
                      (self#attach_std_item_attrs stdAttrs) @@
                        (label ~space:true
                           (makeList ~postSpace:true
                              [atom "module"; atom pmd_name.txt; atom "="])
                           (self#longident_loc alias))
                       in
                    self#attachDocAttrsToLayout ~stdAttrs ~docAttrs
                      ~loc:(pmd_name.loc) ~layout ()
                | Psig_module pmd ->
                    let { stdAttrs; docAttrs } =
                    ...
                    let letPattern =
                      makeList
                        [makeList ~postSpace:true
                           [atom "module"; atom (pmd.pmd_name).txt];
                        atom ":"]
                       in
                    let layout =
                      (self#attach_std_item_attrs stdAttrs) @@
                ...
                | Psig_include incl ->
                    ...
                    let layout =
                      (self#attach_std_item_attrs stdAttrs) @@
                       ...
                       in
                    self#attachDocAttrsToLayout ~stdAttrs ~docAttrs
                      ~loc:((incl.pincl_mod).pmty_loc) ~layout ()
                | Psig_modtype x ->
                    ...
                    let main =
                      ...
                      | Some mt -> self#module_type letPattern mt  in
                    let { stdAttrs; docAttrs } =
                      partitionAttributes ~partDoc:true x.pmtd_attributes  in
                    let layout = self#attach_std_item_attrs stdAttrs main  in
                    self#attachDocAttrsToLayout ~stdAttrs ~docAttrs
                      ~loc:((x.pmtd_name).loc) ~layout ()
                ...
                | Psig_recmodule decls ->
                    let first xx =
                      ...
                      let letPattern =
                        makeList
                          [makeList ~postSpace:true
                             [atom "module rec"; atom (xx.pmd_name).txt];
                          atom ":"]
                         in
                      let layout =
                        (self#attach_std_item_attrs stdAttrs) @@
                         ...
                         in
                      self#attachDocAttrsToLayout ~stdAttrs ~docAttrs
                        ~loc:((xx.pmd_name).loc) ~layout ()
                       in
                    let notFirst xx =
                      let andLetPattern =
                        makeList
                          [makeList ~postSpace:true
                             [atom "and"; atom (xx.pmd_name).txt];
                          atom ":"]
                         in
                    ...
                    let moduleBindings =
                      ...
                      | hd::tl -> (first hd) :: (List.map notFirst tl)  in
                ...
                | Psig_extension ((({ loc },_) as ext),attrs) ->
                    ...
                    let layout =
                      self#attach_std_item_attrs stdAttrs
                        (self#item_extension ext)
                       in
                    self#attachDocAttrsToLayout ~stdAttrs ~docAttrs ~loc
                      ~layout () : Layout.t)
                in
             source_map ~loc:(x.psig_loc) item : Layout.t)
          method non_arrowed_module_type ?(space= true)  letPattern x =
            match x.pmty_desc with
            | Pmty_alias li ->
                label ~space letPattern
                  (formatPrecedence
                     (label ~space:true (atom "module")
                        (self#longident_loc li)))
            | Pmty_typeof me ->
                let labelWithoutFinalWrap =
                  label ~space
                    (label ~space:true letPattern
                       (makeList ~inline:(false, false) ~wrap:("(", "")
                          [atom "module type of"])) (self#module_expr me)
                   in
          ...
          method simple_module_type ?(space= true)  letPattern x =
            match x.pmty_desc with
            | Pmty_ident li ->
                label ~space letPattern (self#longident_loc li)
            | Pmty_signature s ->
                let items =
                  groupAndPrint ~xf:(self#signature_item)
                    ~getLoc:(fun x  -> x.psig_loc) ~comments:(self#comments)
                ...
                let shouldBreakLabel =
                  if (List.length s) > 1 then `Always else `Auto  in
                label ~indent:0 ~break:shouldBreakLabel
                  (makeList
                     [label ~break:shouldBreakLabel
                        (makeList ~postSpace:true [letPattern; atom "{"])
                        (source_map ~loc:(x.pmty_loc)
                           (makeList
                              ~break:(if (List.length s) > 1
                                      then Always
                                      else IfNeed) ~inline:(true, true)
                              ~postSpace:true ~sep:(SepFinal (";", ";"))
                              items))]) (atom "}")
            | Pmty_extension (s,e) ->
                label ~space letPattern (self#payload "%" s e)
            | _ ->
                makeList ~break:IfNeed ~wrap:("", ")")
                  [self#module_type ~space:false
                     (makeList ~pad:(false, true) ~wrap:("", "(")
                        [letPattern]) x]
              ...
              | Pmty_functor _ ->
                  let rec extract_args args xx =
                    ...
                    | Pmty_functor (_,None ,mt2) ->
                        extract_args (`Unit :: args) mt2
                    | Pmty_functor (s,Some mt1,mt2) ->
                        let arg =
                          if s.txt = "_"
                          then self#module_type ~space:false (atom "") mt1
                          else
                            self#module_type ~space
                              (makeList [atom s.txt; atom ":"]) mt1
                           in
                        extract_args ((`Arg arg) :: args) mt2
                    | _ ->
                        let prepare_arg =
                          function | `Unit -> atom "()" | `Arg x -> x  in
                        let args =
                          match args with
                          | `Unit::[] -> []
                          | xs -> List.rev_map prepare_arg args  in
                        (args, (self#module_type (atom "") xx))
                     in
                  let (args,ret) = extract_args [] x  in
                  label ~space letPattern
                    (makeList ~break:IfNeed ~sep:(Sep "=>") ~preSpace:true
                       ~inline:(true, true) [makeTup args; ret])
              | Pmty_with (mt,l) ->
                  ...
                  let with_constraint =
                    function
                    | Pwith_type (li,td) ->
                        self#formatOneTypeDef typeAtom
                    ...
                    | Pwith_typesubst td ->
                        self#formatOneTypeDef typeAtom
                          (atom ~loc:((td.ptype_name).loc)
                             (td.ptype_name).txt) destrAtom td
                  ...
                  (match l with
                   ...
                   | _ ->
                       label ~space letPattern
                         (label ~space:true
                            (makeList ~preSpace:true
                               [self#module_type ~space:false (atom "") mt;
                               atom "with"])
                            (makeList ~break:IfNeed ~inline:(true, true)
                               ~sep:(Sep "and") ~postSpace:true
                               ~preSpace:true (List.map with_constraint l))))
              | _ -> self#non_arrowed_module_type ~space letPattern x  in
          ...
          method simple_module_expr ?(hug= false)  x =
            ...
            | Pmod_unpack e ->
                let exprLayout =
                  match e.pexp_desc with
                  | Pexp_constraint
                      (e,{ ptyp_desc = Ptyp_package (lid,cstrs) }) ->
                      formatTypeConstraint
                        (makeList ~postSpace:true
                           [atom "val"; self#unparseExpr e])
                        (self#typ_package ~mod_prefix:false lid cstrs)
                  | _ ->
                      makeList ~postSpace:true
                        [atom "val"; self#unparseExpr e]
            ...
            | Pmod_constraint (unconstrainedRet,mt) ->
                let letPattern =
                  makeList [self#module_expr unconstrainedRet; atom ":"]  in
                formatPrecedence (self#module_type letPattern mt)
            | Pmod_structure s ->
                let wrap = if hug then ("({", "})") else ("{", "}")  in
                let items =
                  groupAndPrint ~xf:(self#structure_item)
                    ~getLoc:(fun x  -> x.pstr_loc) ~comments:(self#comments)
                    s
                   in
                makeList ~break:Always_rec ~inline:(true, false) ~wrap
                  ~postSpace:true ~sep:(SepFinal (";", ";")) items
            | _ -> formatPrecedence (self#module_expr x)
          method module_expr x =
            match x.pmod_desc with
            | Pmod_functor _ ->
                let (argsList,return) =
                  self#curriedFunctorPatternsAndReturnStruct x  in
                self#wrapCurriedFunctionBinding funToken ~sweet:true
                  ~arrow:"=>" (makeTup argsList) []
            ...
            | first::_ as structureItems ->
                ...
                let items =
                  groupAndPrint ~xf:(self#structure_item)
                   ...
                   in
                source_map ~loc:{ loc_start; loc_end; loc_ghost = false }
                  (makeList ~postSpace:true ~break:Always_rec ~indent:0
                     ~inline:(true, false) ~sep:(SepFinal (";", ";")) items)
          ...
          method structure_item term =
            let item =
              ...
              | Pstr_eval (e,attrs) ->
                  let { stdAttrs; jsxAttrs; uncurried } =
                    partitionAttributes attrs  in
                  (if uncurried
                   then Hashtbl.add uncurriedTable e.pexp_loc true;
                   (let layout =
                      self#attach_std_item_attrs stdAttrs
                    ...
                    | [] -> layout
                    | _::_ ->
                        let jsxAttrNodes = List.map self#attribute jsxAttrs
                           in
              ...
              | Pstr_module x ->
                  let bindingName =
                    atom ~loc:((x.pmb_name).loc) (x.pmb_name).txt  in
              ...
              | Pstr_include incl ->
                  (self#attach_std_item_attrs incl.pincl_attributes) @@
                    (let moduleExpr = incl.pincl_mod  in
                     self#moduleExpressionToFormattedApplicationItems
                       ~prefix:"include" moduleExpr)
              | Pstr_recmodule decls ->
                  let first xx =
                    (self#attach_std_item_attrs xx.pmb_attributes) @@
                      (self#let_module_binding "module rec"
                         (atom (xx.pmb_name).txt) xx.pmb_expr)
                     in
                  let notFirst xx =
                    (self#attach_std_item_attrs xx.pmb_attributes) @@
                      (self#let_module_binding "and" (atom (xx.pmb_name).txt)
                         xx.pmb_expr)
                     in
                  let moduleBindings =
                    match decls with
              ...
              | Pstr_extension ((extension,PStr (item::[])),a) ->
                  (match item.pstr_desc with
                   ...
                   | _ ->
                       let { stdAttrs; docAttrs } =
                         partitionAttributes ~partDoc:true a  in
                       let layout =
          ...
          method type_extension te =
            let formatOneTypeExtStandard prepend ({ ptyext_path } as te) =
              ...
              let item = self#formatOneTypeExt prepend name (atom "+=") te
                 in
              let { stdAttrs; docAttrs } =
                partitionAttributes ~partDoc:true te.ptyext_attributes  in
              let layout = self#attach_std_item_attrs stdAttrs item  in
              self#attachDocAttrsToLayout ~stdAttrs ~docAttrs
                ~loc:(ptyext_path.loc) ~layout ()
               in
            ...
            let case_row { pc_lhs; pc_guard; pc_rhs } =
              ...
              let appendWhereAndArrow p =
                ...
                | Some g ->
                    let withWhen =
                      label ~space:true p
                        (makeList ~break:Layout.Never ~inline:(true, true)
                           ~postSpace:true
                           [label ~space:true (atom "when")
                              (self#unparseExpr g)])
                       in
                    makeList ~inline:(true, true) ~postSpace:true
                      [withWhen; atom "=>"]
              ...
              let orsWithWhereAndArrowOnLast =
                appendWhereAndArrowToLastOr theOrs  in
              let rhs =
              ...
              source_map
               ...
               in
            groupAndPrint ~xf:case_row
              ~getLoc:(fun { pc_lhs; pc_rhs }  ->
                         {
                           (pc_lhs.ppat_loc) with
                           loc_end = ((pc_rhs.pexp_loc).loc_end)
                         }) ~comments:(self#comments) l
          method singleArgParenApplication ?(uncurried= false)  es =
            ...
            | ({ pexp_attributes = []; pexp_desc } as exp)::[] when
                is_simple_list_expr exp ->
                (match view_expr exp with
                 | `list xs ->
                     self#unparseSequence ~construct:`List
                       ~wrap:(lparen, ")") xs
                 | `cons xs ->
                     self#unparseSequence ~construct:`ES6List
                       ~wrap:(lparen, ")") xs
                 | _ -> assert false)
            | _ -> assert false
          ...
          method formatFunAppl ~jsxAttrs  ~args  ~funExpr  ~applicationExpr 
            ...
            let categorizeFunApplArgs args =
              ...
              | ((_,{ pexp_desc = Pexp_fun _ }) as callback)::args when
                  [] ==
                    (List.filter
                       (fun (_,e)  ->
                          match e.pexp_desc with
                ...
                (if uncurried
                 ...
                 (let (cbArgs,retCb) =
                  ...
                  let theCallbackArg =
                    ...
                    | Optional s ->
                        makeList
                          ([atom namedArgSym; atom s; atom "=?"] @ [cbArgs])
                    | Labelled s ->
                        makeList
                          ([atom namedArgSym; atom s; atom "="] @ [cbArgs])
                    | Nolabel  -> cbArgs  in
                  let theFunc =
                    source_map ~loc:(funExpr.pexp_loc)
                      (makeList
                         ~wrap:("",
                                 (if uncurriedApplication then "(." else "("))
                         [formattedFunExpr])
                     in
                  let formattedFunAppl =
                    ...
                    | x::[] ->
                        ...
                        let left =
                          label theFunc
                            (makeList ~pad:(uncurriedApplication, false)
                               ~wrap:("", " ") ~break:IfNeed
                               ~inline:(true, true) ~sep:(Sep ",")
                               ~postSpace:true argsWithCallbackArgs)
                           in
                        label left returnValueCallback
                    | xs ->
                        let printWidthExceeded =
                          Reason_heuristics.funAppCallbackExceedsWidth
                            ~printWidth:(settings.width) ~args ~funExpr ()
                           in
                        if printWidthExceeded = false
                        then
                          let right =
                            source_map ~loc:(retCb.pexp_loc)
                              (makeList ~break:Always_rec
                                 ~wrap:("=> {", "})")
                                 ~sep:(SepFinal (";", ";")) xs)
                             in
                          let argsWithCallbackArgs =
            ...
            | `NormalFunAppl args ->
                let theFunc =
                  source_map ~loc:(funExpr.pexp_loc) formattedFunExpr  in
                let (syntheticApplicationLocation,syntheticArgLoc) =
                  ...
                  | hd::_ ->
                   ...
                   in
                let theArgs =
                  (self#reset)#label_x_expression_params ~uncurried args  in
                maybeJSXAttr @
                  [source_map ~loc:syntheticApplicationLocation
      ...
      let toplevel_phrase ppf x =
        ...
        | Ptop_dir (s,da) ->
            print_string "(* top directives not supported *)"
      ...
      let longident_for_arity lid =
        let rec toplevel =
          ...
          | Lident s -> [s]
          | Ldot (lid,s) ->
              let append_s x = x ^ ("." ^ s)  in s ::
                (List.map append_s (toplevel lid))
          | Lapply (y,s) -> toplevel s  in
        match lid with
      ...
      let add_explicit_arity_mapper super =
        ...
        let expr mapper expr =
          let expr =
            match expr with
            | { pexp_desc = Pexp_construct (lid,Some sp); pexp_loc;
                ...
                {
                  pexp_desc =
                    (Pexp_construct (lid, (Some (wrap_expr_with_tuple sp))));
                  pexp_loc;
                  pexp_attributes =
                    (add_explicit_arity pexp_loc pexp_attributes)
                }
            | x -> x  in
        ...
        and pat mapper pat =
          let pat =
            match pat with
            | { ppat_desc = Ppat_construct (lid,Some sp); ppat_loc;
                ...
                {
                  ppat_desc =
                    (Ppat_construct (lid, (Some (wrap_pat_with_tuple sp))));
                  ppat_loc;
                  ppat_attributes =
                    (add_explicit_arity ppat_loc ppat_attributes)
                }
            | x -> x  in
          super_pat mapper pat
         in { super with Ast_mapper.expr = expr; pat } 
