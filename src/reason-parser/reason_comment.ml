...
let dump ppf t =
  Format.fprintf ppf "%d (%d:%d)-%d (%d:%d) -- %s:||%s||"
    ((t.location).loc_start).pos_cnum ((t.location).loc_start).pos_lnum
    (((t.location).loc_start).pos_cnum - ((t.location).loc_start).pos_bol)
    ((t.location).loc_end).pos_cnum ((t.location).loc_end).pos_lnum
    (((t.location).loc_end).pos_cnum - ((t.location).loc_end).pos_bol)
    (string_of_category t.category) t.text
