...
type print_itype =
  [ `ML  | `Reason  | `Binary  | `BinaryReason  | `AST  | `None ]
...
module type PRINTER  =
  sig
    ...
    val parse :
      use_stdin:bool ->
        parse_itype -> string -> ((t * Reason_comment.t list) * bool)
    val print :
      print_itype ->
        string ->
          bool ->
            out_channel ->
              Format.formatter -> (t * Reason_comment.t list) -> unit
  end
let err s = raise (Invalid_config s) 
let prepare_output_file =
  function
...
let ocamlBinaryParser use_stdin filename =
  let chan =
    ...
    | true  -> stdin
    | false  ->
        let file_chan = open_in filename  in (seek_in file_chan 0; file_chan)
     in
...
let reasonBinaryParser use_stdin filename =
  let chan =
    ...
    | false  ->
        let file_chan = open_in filename  in (seek_in file_chan 0; file_chan)
     in
  let (magic_number,filename,ast,comments,parsedAsML,parsedAsInterface) =
    input_value chan  in
