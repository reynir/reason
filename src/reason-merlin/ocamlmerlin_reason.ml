...
module Reason_reader =
  struct
    ...
    let for_completion t pos =
      ...
      Misc.try_finally (fun ()  -> ({ complete_labels = true }, (parse t)))
    ...
    let pretty_print ppf =
      let open Reason_toolchain in
        let open Reason_pprint_ast in
          ...
          | Pretty_case_list x ->
              (formatter ())#case_list ppf
                (List.map From_current.copy_case x)
          | Pretty_expression x ->
          ...
          | Pretty_toplevel_phrase x ->
              (formatter ())#toplevel_phrase ppf
                (From_current.copy_toplevel_phrase x)
...
let () =
  let open Extend_main in
    extension_main
      ~reader:(Reader.make_v0 ((module Reason_reader) : (module V0)))
