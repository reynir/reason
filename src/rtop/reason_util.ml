...
let extract_exn src name =
  try
    ...
    assert false
  with | exn -> (assert ((Printexc.exn_slot_name exn) = name); exn) 
...
let correctly_catch_parse_errors fn lexbuf =
  let kind =
    if (!Toploop.input_name) = "//toplevel//" then `Toplevel else `Batch  in
  try fn lexbuf
  with
  | exn when kind = `Toplevel ->
      raise
        (match exn with
         ...
         | Reason_syntax_util.Error (loc,_) ->
             transmogrify_exn (Syntaxerr.Error (Syntaxerr.Other loc))
               exn_Syntaxerr_Error
         | _ -> exn)
